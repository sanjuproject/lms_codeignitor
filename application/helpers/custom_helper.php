...
function getchild_diagnosticexam_status($child_id)
{
	...
	if (!empty($courseresult)) {
		...
		$lastassessment_exam = $CI->ss_aw_assessment_exam_completed_model->get_last_exam_details($child_id, $type); // Search LAST Assessment Exam code
	}
	$last_exam_code = "";
	...
	if (empty($check_purchase_course_exist)) {
		...
		if (!empty($courseresult)) {
			if ($courseresult[count($courseresult) - 1]['ss_aw_course_status'] == 2) {
				$purchase_date = date('Y-m-d', strtotime($courseresult[count($courseresult) - 1]['ss_aw_child_course_modified_date']));
...
function emailnotification_new($email, $subject, $msg, $cc = "", $attachment = "")
{
...
function emailnotification($email, $subject, $msg, $cc = "", $attachment = "", $bcc = "")
{
	$unsubscribe_link = base_url() . 'unsubscribe/' . base64_encode($email);
	$msg = str_ireplace("[@@UNSUBSCRIBE_USER_EMAIL_LINK@@]", $unsubscribe_link, $msg);
	$msg = str_ireplace("[@@CURRENT_YEAR@@]", date('Y'), $msg);
	...
	$api_key = "3b246c24a1af2a042ab05f4c80ead807";
	$endpoint = "http://smsc.biz/httpemailapi/v1/send";

	$curl = curl_init();

	$data = array();
	$data['api_key'] = $api_key;
	$data['from'] = "swagat@alsowise.com";
	if (!empty($cc)) {
		$data['cc'] = $cc;
	}
	if (!empty($bcc)) {
	...
	if (!empty($attachment)) {
		...
		$post_fields = array(
			$file_field => curl_file_create($file_path, $file_mime, $file_name),
		) + $data;
	} else {
		$post_fields = $data;
	}
...
function getlastlessondata($instution_id, $lesson_id, $course)
{
	$ci = get_instance();
	$ci->load->model('module_wise_performance_model');
	$rest = $ci->module_wise_performance_model->getlessondata($instution_id, $lesson_id, $course);	
	return $rest->val;
}
