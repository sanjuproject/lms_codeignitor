...
function get_active_inactive_delinquent_students($data = array()) {
    ...
    $get_lesson_complete_students = $ci->ss_aw_child_last_lesson_model->check_student_status($data); //lesson start date end date child wise
    if (!empty($get_lesson_complete_students)) {
        foreach ($get_lesson_complete_students as $key => $value) {

            $child_id = $value->child_id;
            if ($value->date_diff != null) {
                if ($value->date_diff > 21) {
                    $inactive_student_ary[] = $value->child_id;
                } elseif ($value->date_diff > 14) {
                    $delinquent_student_ary[] = $value->child_id;
                } elseif ($value->date_diff > 7) {
                    $active_student_ary[] = $value->child_id;
                }
            }
        }
    }

    $data['all_childs_ary'] = $all_childs_ary; //child id's
...
function get_delinquent_students($data = array()) {
    ...
    $delinquent_student_ary['first'] = array();
    $delinquent_student_ary['second'] = array();
    $delinquent_student_ary['third'] = array();
    $delinquent_student_ary['last'] = array();
    if (!empty($get_lesson_complete_students)) {
        foreach ($get_lesson_complete_students as $key => $value) {
            $child_id = $value->child_id;
...
//				$diffDay = round($diff / (60 * 60 * 24));
            if ($value->date_diff >= 1 && $value->date_diff < 8) {
                $delinquent_student_ary['first'][] = $value->child_id;
            } elseif ($value->date_diff >= 8 && $value->date_diff < 15) {
                $delinquent_student_ary['second'][] = $value->child_id;
            } elseif ($value->date_diff > 15) {
