...
<script>
    ...
    $('body').on('click', '.buy_now', function(e) {
        ...
        if (payment_amount == 0) {
            ...
            $("#overlay").fadeIn(300);
            $.ajax({
                url: '<?php echo base_url() ?>' + 'diagnostic_institution/bulk_payment',
                type: 'post',
                dataType: 'json',
                data: {
                    course_id: course_id,
                ...
                success: function(data) {
                    var url = '<?php echo base_url() ?>' + 'diagnostic_institution/paymentsuccess/' + data.status;
                    window.location = url;
                    // location.replace(url);
                },
                error: function(xhr, ajaxOptions, thrownError) {                  
                    var url = '<?php echo base_url() ?>' + 'diagnostic_institution/paymentsuccess';
                    window.location = url;
                    //location.replace(url);
                }
            });
