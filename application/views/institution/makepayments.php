...
$('body').on('click', '.buy_now', function(e){
    ...
    if (payment_amount == 0) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {
                course_id: course_id, transaction_id: razorpay_payment_id , payment_amount: payment_amount, discount_amount: discount_amount, coupon_code: coupon_code, payment_type: payment_type, institution_id: institution_id, excel_upload_id: excel_upload_id
            }, 
            success: function (data) {
                var url = '<?php echo base_url() ?>'+'institution/paymentsuccess/'+data.status;
                window.location = url;
                //location.replace(url);
            },
            error: function (xhr, ajaxOptions, thrownError) {
                var url = '<?php echo base_url() ?>'+'institution/paymentsuccess';
                window.location = url;
                //location.replace(url);
            }
        });
        ...
        var options = {
            ...
            "handler": function (response){
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        course_id: course_id, transaction_id: response.razorpay_payment_id , payment_amount: payment_amount, discount_amount: discount_amount, coupon_code: coupon_code, payment_type: payment_type, institution_id: institution_id, excel_upload_id: excel_upload_id
                    }, 
                    success: function (data) {
                        var url = '<?php echo base_url() ?>'+'institution/paymentsuccess/'+data.status;
                        window.location = url;
