            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="page-title parsley-examples">Manage Payments</h4>
                    </div>
                </div>
            </div>
            <!-- end page title -->
                                        ...
                                        if (!empty($result)) {
                                            foreach ($result as $key => $value) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td><a href="<?= base_url(); ?>institution/upload_user_list/<?= $value->ss_aw_id; ?>"><?= $value->ss_aw_student_number; ?></a></td>
                                                    <td><?= date('d/m/Y', strtotime($value->ss_aw_created_at)); ?></td>
                                                    <td>
                                                        <?= $value->ss_aw_program_type == 1 ? Winners : Master."s"; ?>
                                                    </td>
                                                    <td>
                                                        <?php
                                                        ...
                                                        if ($value->ss_aw_payment_type == 1) {
                                                            echo $value->ss_aw_partial_payment == 0 ? "Lumpsum" : "Lumpsum (".$value->transaction_count."/2)";
                                                        }
                                                    ...
                                                    <td>
                                                        <?php
                                                        if ($value->transaction_count > 0) {
                                                            if ($value->ss_aw_payment_type == 1) {
                                                                if ($value->ss_aw_partial_payment == 1) {
                                                                    if ($value->total_paid_amount > $value->first_paid_amount) {
                                                                        ?>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-4">
                            <div class="formWrapper">
                                <form class="parsley-examples" method="post" id="demo-form" novalidate="" action="<?= base_url(); ?>institution/add_pan_gst">
...
<script type="text/javascript">
    ...
    function makePayment(uploadId){
        $.ajax({
            type:"POST",
            url:"<?= base_url(); ?>institution/check_pan_gst",
            data:{"excel_upload_id": uploadId},
            dataType:"JSON",
            success:function(data){
                console.log(data);
                if (data.status == 200) {
                    var link = "<?= base_url(); ?>institution/make_payment/"+uploadId;
