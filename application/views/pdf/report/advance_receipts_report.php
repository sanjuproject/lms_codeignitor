...
<body>
	<table class="table table-centered table-striped dt-responsive nowrap w-100" data-show-columns="true">
        ...
        <tbody>
            <?php
            foreach ($reporting_collection as $key => $value) {
            ?>
            <tr>
                                                                ...
                                                                if (!empty($reporting_revenue[$value->ss_aw_parent_id][date('m', strtotime($value->ss_aw_created_at))])) {
                                                                    echo $invoice_amount = $value->ss_aw_invoice_amount - $reporting_revenue[$value->ss_aw_parent_id][date('m', strtotime($value->ss_aw_created_at))];
                                                                }
                                                                else{
                                                                    echo $invoice_amount = $value->ss_aw_invoice_amount;
                                                                }
                                                                ?></td>
                                                                <td><?= $value->ss_aw_payment_type == 0 ? "Lumpsum" : "EMI" ?></td>
                                                                <td><?= $value->ss_aw_discount_amount; ?></td>
                                                                <td><?= $gst = round(($invoice_amount*18)/100); ?></td>
                                                                <td><?= $invoice_amount + $gst; ?></td>
                                                            </tr>
            <?php
            }
            ?>
