...
class Store_procedure_model extends CI_Model
{
    ...
    public function get_total_combine_score_by_topic($child_id, $level, $topic){
        $this->db->where('ss_aw_lesson_topic', $topic);
        $this->db->where('ss_aw_child_id', $child_id);
        $this->db->where('ss_aw_level', $level);
        return $this->db->get('ss_aw_lesson_assessment_confidence')->result();
    }
    ...
    public function getquestioncompletetimebytopic($child_id, $level, $topic){
        $this->db->where('ss_aw_child_id', $child_id);
        $this->db->where('ss_aw_level', $level);
        $this->db->where('ss_aw_topic_name', $topic);
        return $this->db->get('ss_aw_assessment_review_answer_timing')->result();
    }
}