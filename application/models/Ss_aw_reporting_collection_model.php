...
class Ss_aw_reporting_collection_model extends CI_Model
{
	...
	public function getalldata($start_date, $end_date){
		...
		$this->db->from($this->table);
		$this->db->join('ss_aw_parents','ss_aw_parents.ss_aw_parent_id = ss_aw_reporting_collection.ss_aw_parent_id and `ss_aw_parents`.`ss_aw_parent_delete` = 0');
		...
		if (count($start_date_ary) > 1 && count($end_date_ary) > 1) {
			...
			$query = "(MONTH(ss_aw_reporting_collection.ss_aw_created_at) BETWEEN $start_month AND $end_month) "
                                . "AND (YEAR(ss_aw_reporting_collection.ss_aw_created_at) BETWEEN $start_year AND $end_year)";
			$this->db->where($query);
		}
		else{
			$month = $start_date;
	...
	public function getcollectionwithrenevue($start_date, $end_date){
		$start_date_ary = explode("/", $start_date);
		$end_date_ary = explode("/", $end_date);
		$this->db->select('ss_aw_reporting_collection.*,ss_aw_parents.ss_aw_parent_full_name,ss_aw_parents.ss_aw_parent_address,ss_aw_parents.ss_aw_parent_city,ss_aw_parents.ss_aw_parent_state,ss_aw_parents.ss_aw_parent_pincode,ss_aw_parents.ss_aw_parent_country,(select group_concat(ss_aw_id) from ss_aw_reporting_revenue where ss_aw_reporting_revenue.ss_aw_bill_no = ss_aw_reporting_collection.ss_aw_bill_no) as revenue_ids');
		$this->db->from($this->table);
		$this->db->join('ss_aw_parents','ss_aw_parents.ss_aw_parent_id = ss_aw_reporting_collection.ss_aw_parent_id');
		if (count($start_date_ary) > 1 && count($end_date_ary) > 1) {
			$start_month = $start_date_ary[0];
