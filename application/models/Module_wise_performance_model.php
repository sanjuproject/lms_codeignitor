...
public function get_module_individual_performence_data($institution_id, $level, $course_id, $institution_list = 0){
    ...
    tot.ss_aw_child_id,
    tot.ss_aw_child_first_name,
    tot.ss_aw_child_last_name,
    tot.ss_aw_diagonastic_exam_date,
    ss_aw_lesson_topic,
    ss_aw_lession_id,
    ss_aw_section_reference_no,
    tot.ss_aw_assessment_id,
            tot.ss_aw_lesson_id,
            lesson_score.total_question,
            lesson_score.wright_answers,
            asses_score.total_question,
            asses_score.wright_answers,
            (case when lesson_score.total_question is null then null else (case when lesson_score.total_question>lesson_score.wright_answers then round((lesson_score.wright_answers*100)/lesson_score.total_question) else 0 end) end)lessonscore,
            (case when asses_score.total_question is null then null else (case when asses_score.total_question>asses_score.wright_answers then round((asses_score.wright_answers*100)/asses_score.total_question) else 0 end) end)assesmentscore
...
FROM
    ...
    WHERE
        ap.ss_aw_institution = '$institution_id'            
            AND ap.ss_aw_parent_delete = '0') child
    JOIN (SELECT 
        ss_aw_lesson_topic,
            ss_aw_lession_id,
            ss_aw_section_reference_no
    ...
    JOIN ss_aw_lessons_uploaded lub ON (lub.ss_aw_lesson_topic_id = ss_aw_section_id)
 ...
 $champ) lesson_listary
    ...
    JOIN ss_aw_assesment_uploaded aau ON aau.ss_aw_assessment_id = qa.ss_aw_assessment_id and aau.ss_aw_assesment_delete=0
    GROUP BY aau.ss_aw_assessment_id) aqa on aqa.ss_aw_lesson_id=lesson_listary.ss_aw_lession_id    
            ) tot
	left join (select * from ss_aw_lesson_score ls group by child_id,lesson_id)lesson_score on lesson_score.lesson_id=tot.ss_aw_lession_id and lesson_score.child_id=tot.ss_aw_child_id
    left join (select * from ss_aw_assessment_score aas group by child_id,assessment_id)asses_score on asses_score.assessment_id=tot.ss_aw_assessment_id and asses_score.child_id=tot.ss_aw_child_id
")->result_array();
	...
	public function getmodulewisereportdata($institution_id, $level, $course_id, $institution_list = 0)
	{
	    ...
	    sum(case when fi.id!='' then 1 else 0 end)assessmentcompletenum,
	     ...
	     sum(case when fi.ss_aw_last_lesson_modified_date!='' then 1 else 0 end)assessmentnonaccessable,
	      sum(case when fi.ss_aw_lession_id='59' then 1 when fi.ss_aw_lession_id!=1 and now() >= accessdate then 1 end)lessonnonaccessable
	FROM
	    (SELECT 
	        tot.*,
