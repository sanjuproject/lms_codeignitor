  ...
  class Ss_aw_schedule_readalong_model extends CI_Model
{
	...
	public function search_byparam($data = array())
	{
		...
		ss_aw_readalongs_upload.ss_aw_status,ss_aw_readalongs_upload.ss_aw_deleted,ss_aw_readalongs_upload.ss_aw_is_demo');
	...
	public function scheduledreadalongdetail($data){
		$this->db->where('ss_aw_child_id', $data['ss_aw_child_id']);
		if (!empty($data['ss_aw_course_id'])) {
			$this->db->where('ss_aw_course_id', $data['ss_aw_course_id']);
		}
		$this->db->order_by('ss_aw_id','desc');
		$this->db->limit(1);
		return $this->db->get($this->table)->result();
  ...
  public function check_active_readalong($child_id, $readalong_id = "") {
    $this->db->where('ss_aw_child_id', $child_id);
    if (!empty($readalong_id)) {
    	$this->db->where('ss_aw_readalong_id ', $readalong_id);	
    }
    $this->db->where('ss_aw_readalong_status !=', 1);
    $this->db->order_by('ss_aw_id', 'desc');
  ...
  public function get_active_readalong_count($child_id, $course_id){
  	$this->db->where('ss_aw_child_id', $child_id);
  	$this->db->where('ss_aw_course_id', $course_id);
  	$this->db->where('ss_aw_readalong_status !=', 1);
  	$this->db->where('ss_aw_read', 1);
  	return $this->db->get($this->table)->num_rows();
  }
  public function get_active_readalong_all_count($child_id, $course_id){
  	$this->db->where('ss_aw_child_id', $child_id);
  	$this->db->where('ss_aw_course_id', $course_id);
  	return $this->db->get($this->table)->num_rows();
  }
