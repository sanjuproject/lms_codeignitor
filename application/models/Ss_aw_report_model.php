...
class Ss_aw_report_model extends CI_Model
{
    ...
    /*public function totallessonquesion($search_data = array()) {
        if ($search_data['start_date'] != '' && $search_data['end_date'] != '') {
            if ($search_data['start_date'] < report_date_from) {
                $search_data['start_date'] = report_date_from;
            }
            $start_date = date_format(date_create($search_data['start_date']), 'Y-m-d');
            $end_date = date_format(date_create($search_data['end_date']), 'Y-m-d');
            $date_filter = " AND DATE(qa.ss_aw_created_date) >= '$start_date'
                AND DATE(qa.ss_aw_created_date) <= '$end_date' ";
        } else {
            $date_filter = '';
        }

        if ($search_data['topics'] != '') {
            $topic = $search_data['topics'];
            $topic_filter = "  and lu.ss_aw_lesson_topic='$topic'";
        } else {
            $topic_filter = '';
        }
        if ($search_data['student_level'] != '') {
            $student_level = $search_data['student_level'] == 1 ? "1','2" : $search_data['student_level'];
            $level_filter = " and cc.ss_aw_course_id in ('$student_level')";
        } else {
            $level_filter = '';
        }


        return $this->db->query("select al.ss_aw_lesson_answers,
    al.ss_aw_lession_id,
    al.ss_aw_lesson_title,
    al.ss_aw_lesson_details,
    al.ss_aw_lesson_record_id,
    al.ss_aw_lesson_format,
    al.ss_aw_course_id,
    lu.ss_aw_course_id AS course_level,
    lu.ss_aw_lesson_topic AS topic_name,
     (CASE
        WHEN al.ss_aw_course_id = 1 THEN 'E'
        WHEN al.ss_aw_course_id = 2 THEN 'C'
        WHEN al.ss_aw_course_id = 3 THEN 'A'
        ELSE 'M' END) question_course_level,
        count(al.ss_aw_lession_id)question_count,
        sum(case when qa.ss_aw_answer_status =1 then 1 else 0 end)correct_answer_count,
     sum(case when qa.ss_aw_answer_status =2 then 1 else 0 end)incorrect_answer_count
        from
(select * from ss_aw_lesson_quiz_ans where ss_aw_answer_status!='3' AND ss_aw_answer_status!='0' group by ss_aw_question_id,ss_aw_child_id)qa
join  `ss_aw_lessons` al on qa.`ss_aw_question_id` = al.`ss_aw_lession_id`
join ss_aw_lessons_uploaded lu on al.`ss_aw_lesson_record_id` = lu.`ss_aw_lession_id` $topic_filter
join (select * from `ss_aw_child_course`  group by ss_aw_child_id,ss_aw_course_id order by ss_aw_child_course_id desc) cc on cc.ss_aw_child_id=qa.ss_aw_child_id $level_filter
JOIN `ss_aw_childs` ac ON ac.`ss_aw_child_id` =cc.`ss_aw_child_id` and ac.ss_aw_child_status='1' and ac.ss_aw_child_delete='0'
where 1 $date_filter
                group by al.ss_aw_lession_id
                ORDER BY al.ss_aw_lesson_created_date DESC")->num_rows();
    }
*/
    public function totallessonquesion($search_data = array())
    {
    ...
    lu.ss_aw_lesson_topic AS topic_name,
     (CASE
        ...
        count(al.ss_aw_lession_id)question_count,
        sum(case when qa.ss_aw_answer_status =1 then 1 else 0 end)correct_answer_count,
     sum(case when qa.ss_aw_answer_status !=1 then 1 else 0 end)incorrect_answer_count
        from
(SELECT 
        ss_aw_lesson_quiz_ans.*
    FROM
        ss_aw_lesson_quiz_ans 
        join ss_aw_child_last_lesson ll 
        on ll.ss_aw_child_id=ss_aw_lesson_quiz_ans.ss_aw_child_id and ll.ss_aw_last_lesson_modified_date!='' and ll.ss_aw_lesson_id=ss_aw_lesson_quiz_ans.ss_aw_lesson_id
    GROUP BY ss_aw_lesson_quiz_ans.ss_aw_question_id , ss_aw_lesson_quiz_ans.ss_aw_child_id)qa
join  `ss_aw_lessons` al on qa.`ss_aw_question_id` = al.`ss_aw_lession_id`
join ss_aw_lessons_uploaded lu on al.`ss_aw_lesson_record_id` = lu.`ss_aw_lession_id` $topic_filter
join (select * from `ss_aw_child_course` group by ss_aw_child_id,ss_aw_course_id order by ss_aw_child_course_id desc) cc on cc.ss_aw_child_id=qa.ss_aw_child_id $level_filter
JOIN `ss_aw_childs` ac ON ac.`ss_aw_child_id` =cc.`ss_aw_child_id` and ac.ss_aw_child_status='1' and ac.ss_aw_child_delete='0'
where 1 $date_filter
    ...
    public function totalassessmentquestion($search_data = array())
    {
        ...
        if ($search_data['start_date'] != '' && $search_data['end_date'] != '') {
            ...
            $start_date = date_format(date_create($search_data['start_date']), 'Y-m-d');
            $end_date = date_format(date_create($search_data['end_date']), 'Y-m-d');
            $multi_date_filter = " AND DATE(ss_aw_assesment_multiple_question_answer.ss_aw_created_at) >= '$start_date'
        AND DATE(ss_aw_assesment_multiple_question_answer.ss_aw_created_at) <= '$end_date'";
            $log_date_filter = " AND DATE(ss_aw_assessment_exam_log.ss_aw_log_created_date) >= '$start_date'
        AND DATE(ss_aw_assessment_exam_log.ss_aw_log_created_date) <= '$end_date'";
        } else {
            $multi_date_filter = '';
            $log_date_filter = '';
        }
        ...
        if ($search_data['topics'] != '') {
            $topic = $search_data['topics'];
            $topic_filter = "  AND ad.`ss_aw_category` = '$topic'";
            $exam_topic = " WHERE ss_aw_assessment_topic='$topic'";
        } else {
            $topic_filter = '';
            $exam_topic = '';
        }
        ...
        if ($search_data['age_level'] == 1) {
            $course_level = " and (ad.`ss_aw_level` = 'C' or ad.`ss_aw_level`='E')";
            $exam_level="and (ss_aw_log_level='C' or  ss_aw_log_level='E')";
            $child_filter = " AND TIMESTAMPDIFF(year,DATE(ac.ss_aw_child_dob), DATE(cc.ss_aw_child_course_create_date))<13";
        } else {
            $course_level = " and ad.`ss_aw_level` = 'C'";
            $exam_level="and ss_aw_log_level='C'";
            $child_filter = " AND TIMESTAMPDIFF(year,DATE(ac.ss_aw_child_dob), DATE(cc.ss_aw_child_course_create_date))>=13";
        }
        ...
        ad.`ss_aw_answers`,
            ...
            ad.`ss_aw_format`,
            ad.`ss_aw_id`,
            ad.`ss_aw_question_preface`,
            ad.ss_aw_sub_category,
            ad.`ss_aw_question`,
            ad.`ss_aw_level`,
            mqa.ss_aw_child_id child_id,
            mqa.ss_aw_level exam_level,
            mqa.ss_aw_answers_status answer_status,
               COUNT( mqa.ss_aw_child_id) question_asked,
    sum(CASE
        WHEN mqa.ss_aw_answers_status = 1 THEN 1 else 0
    END) correct_answer,
   sum(CASE
        WHEN mqa.ss_aw_answers_status = 2 THEN 1 else 0
    END) wrong_answer
    FROM (select * from ss_aw_assesment_multiple_question_answer where ss_aw_assesment_multiple_question_answer.ss_aw_answers_status != '3'
            AND ss_aw_assesment_multiple_question_answer.ss_aw_answers_status != '0' $multi_date_filter 
        group by ss_aw_question_id,ss_aw_child_id)mqa
    join `ss_aw_assisment_diagnostic` ad 
    on mqa.ss_aw_question = ad.ss_aw_question and ad.`ss_aw_format`='Multiple' 
    and ad.`ss_aw_question` != '' $topic_filter  $course_level
    
      
    JOIN (select * from `ss_aw_child_course`  
    group by ss_aw_child_id,ss_aw_course_id 
    order by ss_aw_child_course_id desc)cc ON cc.ss_aw_child_id = mqa.ss_aw_child_id $level_filter
        join ss_aw_childs ac on ac.ss_aw_child_id=cc.ss_aw_child_id 
        AND ac.ss_aw_child_delete='0' AND ac.ss_aw_child_status='1' $child_filter
    WHERE
       1
           group by mqa.ss_aw_question_id order by ad.ss_aw_sub_category)multiple_data
                UNION 
               select * from (SELECT 
        ad.`ss_aw_answers`,
          ad.`ss_aw_seq_no`,
          ad.`ss_aw_format`,
           ...
           ad.`ss_aw_question`,
           ad.`ss_aw_level`,
            ael.ss_aw_log_child_id child_id,
            ael.ss_aw_log_level exam_level,
            ael.ss_aw_log_answer_status answer_status,
              COUNT( ael.ss_aw_log_child_id) question_asked,
    sum(CASE
        WHEN ael.ss_aw_log_answer_status = 1 THEN 1 else 0
    END) correct_answer,
   sum(CASE
        WHEN ael.ss_aw_log_answer_status = 2 THEN 1 else 0
    END) wrong_answer
    FROM
    (select * from ss_aw_assessment_exam_completed $exam_topic group by ss_aw_assessment_topic,ss_aw_child_id)ec 
   join (select * from (SELECT * FROM ss_aw_assessment_exam_log where  ss_aw_log_answer_status != '3' AND ss_aw_log_answer_status != '0' $log_date_filter $exam_level GROUP BY ss_aw_log_exam_code,ss_aw_log_question_id , ss_aw_log_child_id)lo group by lo.ss_aw_log_question_id , lo.ss_aw_log_child_id)ael on ael.ss_aw_log_exam_code=ec.ss_aw_exam_code
    join  `ss_aw_assisment_diagnostic` ad on ael.ss_aw_log_question_id = ad.ss_aw_id and ad.`ss_aw_format`='Single' $topic_filter  $course_level
     JOIN (select * from `ss_aw_child_course`  group by ss_aw_child_id,ss_aw_course_id order by ss_aw_child_course_id desc)cc ON cc.ss_aw_child_id = ael.ss_aw_log_child_id $level_filter
        join ss_aw_childs ac on ac.ss_aw_child_id=cc.ss_aw_child_id AND ac.ss_aw_child_delete='0' AND ac.ss_aw_child_status='1' $child_filter
    where 1
 group by ael.ss_aw_log_question_id order by ad.ss_aw_sub_category)singale_data")->num_rows();
    }

    /*public function totaldiagnosticquestion($search_data = array()) {
        if ($search_data['start_date'] != '' && $search_data['end_date'] != '') {
            if ($search_data['start_date'] < '2022-08-01') {
                $search_data['start_date'] = '2022-08-01';
            }
            $start_date = date_format(date_create($search_data['start_date']), 'Y-m-d');
            $end_date = date_format(date_create($search_data['end_date']), 'Y-m-d');
            $date_filter = " AND DATE(el.ss_aw_diagonastic_log_created_date) >= '$start_date'
        AND DATE(el.ss_aw_diagonastic_log_created_date) <= '$end_date'";
        } else {
            $date_filter = '';
        }
        if (!empty($search_data['student_level'])) {
            $student_level = $search_data['student_level'] == 1 ? "1','2" : $search_data['student_level'];
            $level_filter = " AND cc.ss_aw_course_id IN ('$student_level')";
        } else {
            $level_filter = '';
        ...
        if ($search_data['topics']) {
            $topic = $search_data['topics'];
            $topics_filter = "  AND ad.ss_aw_category = '$topic'";
        } else {
            $topics_filter = '';
        }
   ...
   ad.`ss_aw_level`,
   cc.`ss_aw_course_id`,
   ad.`ss_aw_category` AS `topic_name`,
    ad.`ss_aw_seq_no`,el.ss_aw_diagonastic_log_child_id,count(ad.ss_aw_id)question_count,
    sum(case when el.ss_aw_diagonastic_log_answer_status =1 then 1 else 0 end)correct_answre,
     sum(case when el.ss_aw_diagonastic_log_answer_status =2 then 1 else 0 end)wrong_answre
FROM
    ss_aw_diagonastic_exam_log el
        JOIN
    ss_aw_assisment_diagnostic ad ON ad.ss_aw_id = el.ss_aw_diagonastic_log_question_id
        $topics_filter
        AND ad.ss_aw_quiz_type = '1'
        AND ad.ss_aw_question != ''
        JOIN
    (select * from `ss_aw_child_course`  group by ss_aw_child_id,ss_aw_course_id order by ss_aw_child_course_id desc)cc 
    ON cc.ss_aw_child_id = el.ss_aw_diagonastic_log_child_id
        $level_filter
            join ss_aw_childs ac on ac.ss_aw_child_id=cc.ss_aw_child_id AND ac.ss_aw_child_delete='0' AND ac.ss_aw_child_status='1'
WHERE el.ss_aw_diagonastic_log_answer_status!='3' 
  $date_filter
        group by ad.ss_aw_id")->num_rows();
    }*/
...
FROM
    (select * from 
    ss_aw_diagonastic_exam_log group by ss_aw_diagonastic_exam_log.ss_aw_diagonastic_log_child_id,ss_aw_diagonastic_exam_log.ss_aw_diagonastic_log_question_id) el
     join (select * from ss_aw_diagonastic_exam group by ss_aw_diagonastic_child_id order by ss_aw_diagonastic_exam_id DESC)de on de.ss_aw_diagonastic_child_id= el.ss_aw_diagonastic_log_child_id 
JOIN
    ss_aw_assisment_diagnostic ad ON ad.ss_aw_id = el.ss_aw_diagonastic_log_question_id
        $topics_filter
        AND ad.ss_aw_quiz_type = '1'
...
WHERE el.ss_aw_diagonastic_log_answer_status!='3' 
  $date_filter
    ...
    public function totalreadalongquestion($child_id = array(), $search_data = array())
    {
        ...
        $this->db->where('ss_aw_readalong_quiz.ss_aw_readalong_upload_id', $search_data['topics']);
        $this->db->where('DATE(ss_aw_readalong_quiz_ans.ss_aw_created_date)>=', report_date_from);
        $this->db->group_by(array('ss_aw_readalong_quiz_ans.ss_aw_quiz_id'));
        return $this->db->get()->num_rows();
    }

    /*public function fetchalllessonsquestions($search_data = array(), $limit, $start) {
...
where 1 $date_filter
                group by al.ss_aw_lession_id
                ORDER BY al.ss_aw_lesson_created_date DESC limit $start,$limit")->result();
    }*/
    ...
    public function fetchalllessonsquestions($search_data = array(), $limit, $start)
    {
        ...
        //         WHEN al.ss_aw_course_id = 3 THEN 'A'
        //         ELSE 'M'
        //     END) question_course_level,
        //     COUNT(al.ss_aw_lession_id) question_count,
        //     SUM(CASE
        //         WHEN qa.ss_aw_answer_status = 1 THEN 1
        //         ELSE 0
        //     END) correct_answer_count,
        //     SUM(CASE
        //         WHEN qa.ss_aw_answer_status != 1 THEN 1
        //         ELSE 0
        //     END) incorrect_answer_count
        //    from ss_aw_lessons_uploaded lu
        //     JOIN
        //     `ss_aw_lessons` al ON al.`ss_aw_lesson_record_id` = lu.`ss_aw_lession_id`
        //     join
        //     ss_aw_lesson_quiz_ans qa ON qa.`ss_aw_question_id` = al.`ss_aw_lession_id`
        //     JOIN
        //     (SELECT 
        //         *
        //     FROM
        //         `ss_aw_child_course`
        //     GROUP BY ss_aw_child_id , ss_aw_course_id
        //     ORDER BY ss_aw_child_course_id DESC) cc ON cc.ss_aw_child_id = qa.ss_aw_child_id
        //         $level_filter
        //         JOIN
        //     `ss_aw_childs` ac ON ac.`ss_aw_child_id` = cc.`ss_aw_child_id`
        //         AND ac.ss_aw_child_status = '1'
        //         AND ac.ss_aw_child_delete = '0'
        //    where 1 $topic_filter $date_filter
        // GROUP BY al.ss_aw_lession_id
        // ORDER BY al.ss_aw_lesson_created_date DESC limit $start,$limit")->result();
        return $this->db->query("select al.ss_aw_lesson_answers,
    al.ss_aw_lession_id,
...
where 1 $date_filter
    ...
    public function fetchallassessmentquestions($search_data = array(), $limit, $start)
    {
   ...
   sum(CASE
    ...
    WHERE
       1
           group by mqa.ss_aw_question_id order by ad.ss_aw_sub_category limit $start,$limit)multiple_data
                UNION 
               select * from (SELECT 
        ad.`ss_aw_answers`,
          ad.`ss_aw_seq_no`,
   ...
   join (select * from (SELECT * FROM ss_aw_assessment_exam_log where  ss_aw_log_answer_status != '3' AND ss_aw_log_answer_status != '0' $log_date_filter $exam_level GROUP BY ss_aw_log_exam_code,ss_aw_log_question_id , ss_aw_log_child_id)lo group by lo.ss_aw_log_question_id , lo.ss_aw_log_child_id)ael on ael.ss_aw_log_exam_code=ec.ss_aw_exam_code
    join  `ss_aw_assisment_diagnostic` ad on ael.ss_aw_log_question_id = ad.ss_aw_id and ad.`ss_aw_format`='Single' $topic_filter  $course_level
     JOIN (select * from `ss_aw_child_course`  group by ss_aw_child_id,ss_aw_course_id order by ss_aw_child_course_id desc)cc ON cc.ss_aw_child_id = ael.ss_aw_log_child_id $level_filter
        join ss_aw_childs ac on ac.ss_aw_child_id=cc.ss_aw_child_id AND ac.ss_aw_child_delete='0' AND ac.ss_aw_child_status='1' $child_filter
    where  1        
 group by ael.ss_aw_log_question_id order by ad.ss_aw_sub_category limit $start,$limit)singale_data")->result();
    }

    /*public function fetchalldiagnosticquestions($search_data = array(), $limit, $start) {
        if ($search_data['start_date'] != '' && $search_data['end_date'] != '') {
            if ($search_data['start_date'] < '2022-08-01') {
                $search_data['start_date'] = '2022-08-01';
            }
...
WHERE el.ss_aw_diagonastic_log_answer_status!='3' 
  $date_filter
        group by ad.ss_aw_id limit $start,$limit")->result();
    }*/
...
WHERE el.ss_aw_diagonastic_log_answer_status!='3' 
  $date_filter
    ...
    public function getreadalongselectedusers($search_key = "", $level, $readalong_title, $type = "")
    {
        //type = 1....Current Year,type = 2.....Current Month
        $current_year = date('Y');
        $current_month = date('m');
        $this->db->select('ss_aw_schedule_readalong.ss_aw_start_date, ss_aw_childs.ss_aw_child_nick_name, ss_aw_childs.ss_aw_child_id, ss_aw_parents.ss_aw_parent_full_name as parent_name, ss_aw_institutions.ss_aw_name as institution_name');
        $this->db->from('ss_aw_schedule_readalong');
        $this->db->join('ss_aw_readalongs_upload', 'ss_aw_schedule_readalong.ss_aw_readalong_id = ss_aw_readalongs_upload.ss_aw_id');
        ...
        $this->db->join('ss_aw_institutions', 'ss_aw_institutions.ss_aw_id = ss_aw_parents.ss_aw_institution', 'left');
        $this->db->where('ss_aw_readalongs_upload.ss_aw_title', $readalong_title);
        $this->db->where('ss_aw_readalongs_upload.ss_aw_level', $level);
        if ($type == 1) {
            $this->db->where('YEAR(ss_aw_schedule_readalong.ss_aw_create_date)', $current_year);
        ...
        if (!empty($search_key)) {
            $this->db->group_start();
            $this->db->like('ss_aw_childs.ss_aw_child_nick_name', $search_key);
            $this->db->or_like('ss_aw_parents.ss_aw_parent_full_name', $search_key);
            $this->db->or_like('ss_aw_institutions.ss_aw_name', $search_key);
            $this->db->group_end();
        }
        return $this->db->get()->result();
    }
    ...
    public function getPTDlessonAssesmentcompletenum()
    {
        ...
        $this->db->group_by(array(
            'YEAR(ss_aw_last_lesson_modified_date)',
            'MONTH(ss_aw_last_lesson_modified_date)',
            'cl.ss_aw_child_id'
        ));
...
MONTH(ss_aw_last_lesson_modified_date)")->row();
    ...
    /*public function get_all_wrong_answers_child($search_data = array(), $child_id = array(), $quiz_type, $question, $question_format = "", $question_id = "") {
        if ($quiz_type == 1) {
            $this->db->select('ss_aw_childs.ss_aw_child_id as child_id, '
                    . 'ss_aw_childs.ss_aw_child_nick_name as child_name, '
                    . 'qa.ss_aw_post_answer as wrong_answer, '
                    . 'ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                    . 'ap.ss_aw_parent_full_name parent_name');
            $this->db->from('(select * from ss_aw_lesson_quiz_ans where ss_aw_answer_status!=3 AND ss_aw_answer_status!=0 group by ss_aw_question_id,ss_aw_child_id)qa');
            $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = qa.ss_aw_child_id '
                    . 'and ss_aw_childs.ss_aw_child_delete=0 and ss_aw_childs.ss_aw_child_status=1');
            $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
            $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
            $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
            $this->db->where('qa.ss_aw_question_id', $question);
            $this->db->where_in('qa.ss_aw_child_id', $child_id);
            $this->db->where('qa.ss_aw_answer_status', 2);
            if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                $this->db->where('qa.ss_aw_created_date >=', $search_data['start_date']);
                $this->db->where('qa.ss_aw_created_date <=', $search_data['end_date']);
            }
            $this->db->group_by('qa.ss_aw_child_id');
            return $this->db->get()->result();
        } elseif ($quiz_type == 2) {
            if ($question_format == 'Multiple') {
                $this->db->select('mqar.ss_aw_answer as wrong_answer,'
                        . ' ss_aw_childs.ss_aw_child_id as child_id, ss_aw_childs.ss_aw_child_nick_name as child_name,'
                        . ' ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                        . 'ap.ss_aw_parent_full_name parent_name');
                $this->db->from('(SELECT * FROM ss_aw_assesment_multiple_question_answer where AND mqa.ss_aw_answers_status != 3 AND mqa.ss_aw_answers_status != 0 GROUP BY ss_aw_question_id , ss_aw_child_id) mqar');
                $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = mqar.ss_aw_child_id');
                $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
                $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
                $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
                $this->db->where('mqar.ss_aw_question', $question);
                $this->db->where('mqar.ss_aw_answers_status', 2);
                $this->db->where_in('mqar.ss_aw_child_id', $child_id);
                if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                    $this->db->where('mqar.ss_aw_created_at >=', $search_data['start_date']);
                    $this->db->where('mqar.ss_aw_created_at <=', $search_data['end_date']);
                }
                $this->db->group_by('ss_aw_assesment_multiple_question_answer.ss_aw_child_id');
                return $this->db->get()->result();
            } elseif ($question_format == 'Single') {
                $this->db->select('ael.ss_aw_log_answers as wrong_answer, '
                        . 'ss_aw_childs.ss_aw_child_id as child_id, ss_aw_childs.ss_aw_child_nick_name as child_name,'
                        . ' ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                        . 'ap.ss_aw_parent_full_name parent_name');
                $this->db->from('(select * from ss_aw_assessment_exam_log GROUP BY ss_aw_assessment_exam_log.ss_aw_log_question_id , ss_aw_assessment_exam_log.ss_aw_log_child_id)ael');
                $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = ael.ss_aw_log_child_id and ss_aw_childs.ss_aw_child_delete=0 and ss_aw_childs.ss_aw_child_status=1');
                $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
                $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
                $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
                $this->db->where('ael.ss_aw_log_question_id', $question_id);
                $this->db->where('ael.ss_aw_log_answer_status', 2);
                $this->db->where('ael.ss_aw_log_answer_status!=', 3);
                $this->db->where('ael.ss_aw_log_answer_status!=', 0);
                $this->db->where_in('ael.ss_aw_log_child_id', $child_id);
                if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                    $this->db->where('ael.ss_aw_log_created_date >=', $search_data['start_date']);
                    $this->db->where('ael.ss_aw_log_created_date <=', $search_data['end_date']);
                }
                $this->db->group_by('ael.ss_aw_log_child_id');
                return $this->db->get()->result();
            }
        } elseif ($quiz_type == 3) {
            $this->db->select('el.ss_aw_diagonastic_log_answers as wrong_answer,ss_aw_childs.ss_aw_child_id as child_id,'
                    . ' ss_aw_childs.ss_aw_child_nick_name as child_name,ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                    . 'ap.ss_aw_parent_full_name parent_name');
            $this->db->from('(select * from  ss_aw_diagonastic_exam_log group by ss_aw_diagonastic_log_child_id,ss_aw_diagonastic_log_question_id)el');
            $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = el.ss_aw_diagonastic_log_child_id and ss_aw_childs.ss_aw_child_delete=0 and ss_aw_childs.ss_aw_child_status=1');
            $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
            $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
            $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
            $this->db->where('el.ss_aw_diagonastic_log_question_id', $question_id);
            $this->db->where('el.ss_aw_diagonastic_log_answer_status', 2);
            $this->db->where_in('el.ss_aw_diagonastic_log_child_id', $child_id);
            $this->db->group_by('el.ss_aw_diagonastic_log_child_id');
            return $this->db->get()->result();
        } else {
...
*/
    public function get_all_wrong_answers_child($search_data = array(), $child_id = array(), $quiz_type, $question, $question_format = "", $question_id = "")
    {
        if ($quiz_type == 1) {
            $this->db->select('ss_aw_childs.ss_aw_child_id as child_id, '
                ...
                . 'ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                . 'ap.ss_aw_parent_full_name parent_name');
            $this->db->from("(select ss_aw_lesson_quiz_ans.* from ss_aw_lesson_quiz_ans "
                . "join ss_aw_child_last_lesson ll "
                . "on ll.ss_aw_child_id=ss_aw_lesson_quiz_ans.ss_aw_child_id and ll.ss_aw_last_lesson_modified_date!='' and ll.ss_aw_lesson_id=ss_aw_lesson_quiz_ans.ss_aw_lesson_id "
                . "where ss_aw_lesson_quiz_ans.ss_aw_answer_status!=3 AND ss_aw_lesson_quiz_ans.ss_aw_answer_status!=0 group by ss_aw_question_id,ss_aw_child_id)qa");
            $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = qa.ss_aw_child_id '
                . 'and ss_aw_childs.ss_aw_child_delete=0 and ss_aw_childs.ss_aw_child_status=1');
            $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
            $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
            ...
            $this->db->where('qa.ss_aw_answer_status', 2);
            if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                $this->db->where('DATE(qa.ss_aw_created_date) >=', $search_data['start_date']);
                $this->db->where('DATE(qa.ss_aw_created_date) <=', $search_data['end_date']);
            }
            $this->db->group_by('qa.ss_aw_child_id');
            return $this->db->get()->result();
        } elseif ($quiz_type == 2) {
            if ($question_format == 'Multiple') {
                ...
                $this->db->where_in('mqar.ss_aw_child_id', $child_id);
                if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                    $this->db->where('DATE(mqar.ss_aw_created_at) >=', $search_data['start_date']);
                    $this->db->where('DATE(mqar.ss_aw_created_at) <=', $search_data['end_date']);
                }
                $this->db->group_by('mqar.ss_aw_child_id');
                return $this->db->get()->result();
            } elseif ($question_format == 'Single') {
                $this->db->select('ael.ss_aw_log_answers as wrong_answer, '
                    . 'ss_aw_childs.ss_aw_child_id as child_id, ss_aw_childs.ss_aw_child_nick_name as child_name,'
                    . ' ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                    . 'ap.ss_aw_parent_full_name parent_name');
                $this->db->from("(select * from ss_aw_assessment_score group by child_id,assessment_id order by id)aas");
                $this->db->join("ss_aw_assessment_exam_log ael", "ael.ss_aw_log_exam_code=aas.exam_code and ael.ss_aw_log_answer_status != '3' and ael.ss_aw_log_answer_status != '0'");
                $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = ael.ss_aw_log_child_id and ss_aw_childs.ss_aw_child_delete=0 and ss_aw_childs.ss_aw_child_status=1');
                $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
                $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
                $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
                ...
                $this->db->where_in('ael.ss_aw_log_child_id', $child_id);
                if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                    $this->db->where('DATE(ael.ss_aw_log_created_date) >=', $search_data['start_date']);
                    $this->db->where('DATE(ael.ss_aw_log_created_date) <=', $search_data['end_date']);
                }
                $this->db->group_by('ael.ss_aw_log_child_id');
                return $this->db->get()->result();
            }
            ...
            $this->db->select('el.ss_aw_diagonastic_log_answers as wrong_answer,ss_aw_childs.ss_aw_child_id as child_id,'
                ...
                . 'ap.ss_aw_parent_full_name parent_name');
            $this->db->from('(select * from  ss_aw_diagonastic_exam_log group by ss_aw_diagonastic_log_child_id,ss_aw_diagonastic_log_question_id)el');
            $this->db->join('(select * from ss_aw_diagonastic_exam group by ss_aw_diagonastic_child_id order by ss_aw_diagonastic_exam_id DESC)de', 'de.ss_aw_diagonastic_child_id= el.ss_aw_diagonastic_log_child_id');
            ...
            $this->db->select('ss_aw_quiz_ans_post as wrong_answer,ss_aw_childs.ss_aw_child_id as child_id,'
                . ' ss_aw_childs.ss_aw_child_nick_name as child_name,ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                . 'ap.ss_aw_parent_full_name parent_name');
            $this->db->from('ss_aw_readalong_quiz_ans');
            $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = ss_aw_readalong_quiz_ans.ss_aw_child_id');
    ...
    /*public function get_all_wright_answers_child($search_data = array(), $child_id = array(), $quiz_type, $question, $question_format = "", $question_id = "") {
        if ($quiz_type == 1) {
            $this->db->select('ss_aw_childs.ss_aw_child_id as child_id, ss_aw_childs.ss_aw_child_nick_name as child_name, '
                    . 'qa.ss_aw_post_answer as wrong_answer, '
                    . 'ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                    . 'ap.ss_aw_parent_full_name parent_name');
            $this->db->from('(select * from ss_aw_lesson_quiz_ans where ss_aw_answer_status!=3 AND ss_aw_answer_status!=0 group by ss_aw_question_id,ss_aw_child_id)qa');
            $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = qa.ss_aw_child_id and ss_aw_childs.ss_aw_child_delete=0 and ss_aw_childs.ss_aw_child_status=1');
            $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
            $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
            $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
            $this->db->where('qa.ss_aw_question_id', $question);
            $this->db->where_in('qa.ss_aw_child_id', $child_id);
            $this->db->where('qa.ss_aw_answer_status', 1);
            if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                $this->db->where('qa.ss_aw_created_date >=', $search_data['start_date']);
                $this->db->where('qa.ss_aw_created_date <=', $search_data['end_date']);
            }
            ...
            if ($question_format == 'Multiple') {
                $this->db->select('mqa.ss_aw_answer as wrong_answer,'
                        . ' ss_aw_childs.ss_aw_child_id as child_id, ss_aw_childs.ss_aw_child_nick_name as child_name, '
                        . 'ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                        . 'ap.ss_aw_parent_full_name parent_name');
                $this->db->from('(select * from ss_aw_assesment_multiple_question_answer where ss_aw_assessment_exam_log.ss_aw_log_answer_status!=3 and ss_aw_assessment_exam_log.ss_aw_log_answer_status!=0 GROUP BY ss_aw_question_id , ss_aw_child_id) mqa');
                $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = mqa.ss_aw_child_id and ss_aw_childs.ss_aw_child_delete=0 and ss_aw_childs.ss_aw_child_status=1');
                $this->db->join('ss_aw_institution_student_upload su', 'su.ss_aw_id=ss_aw_childs.ss_aw_institution_user_upload_id', 'left');
                $this->db->join('ss_aw_institutions ai', 'ai.ss_aw_id=su.ss_aw_institution_id', 'left');
                $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
                $this->db->where('mqa.ss_aw_question', $question);
                $this->db->where('mqa.ss_aw_answers_status', 1);
                $this->db->where_in('mqa.ss_aw_child_id', $child_id);
                if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                    $this->db->where('mqa.ss_aw_created_at >=', $search_data['start_date']);
                    $this->db->where('mqa.ss_aw_created_at <=', $search_data['end_date']);
                }
                $this->db->group_by('mqa.ss_aw_child_id');
                return $this->db->get()->result();
            } elseif ($question_format == 'Single') {
                $this->db->select('ael.ss_aw_log_answers as wrong_answer,'
                        . ' ss_aw_childs.ss_aw_child_id as child_id, ss_aw_childs.ss_aw_child_nick_name as child_name, '
                ...
                $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
                $this->db->where('ael.ss_aw_log_question_id', $question_id);
                $this->db->where('ael.ss_aw_log_answer_status', 1);
                $this->db->where_in('ael.ss_aw_log_child_id', $child_id);
                if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                    $this->db->where('ael.ss_aw_log_created_date >=', $search_data['start_date']);
                    $this->db->where('ael.ss_aw_log_created_date <=', $search_data['end_date']);
            ...
            $this->db->select('el.ss_aw_diagonastic_log_answers as wrong_answer,'
                    . 'ss_aw_childs.ss_aw_child_id as child_id, ss_aw_childs.ss_aw_child_nick_name as child_name,'
                    . 'ss_aw_childs.ss_aw_parent_id as parent_id,ai.ss_aw_name institution_name,'
                    . 'ap.ss_aw_parent_full_name parent_name');
            $this->db->from('(select * from  ss_aw_diagonastic_exam_log group by ss_aw_diagonastic_log_child_id,ss_aw_diagonastic_log_question_id)el');
            ...
            $this->db->join('ss_aw_parents ap', 'ap.ss_aw_parent_id=ss_aw_childs.ss_aw_parent_id', 'left');
            $this->db->where('el.ss_aw_diagonastic_log_question_id', $question_id);
            $this->db->where('el.ss_aw_diagonastic_log_answer_status', 1);
            $this->db->where_in('el.ss_aw_diagonastic_log_child_id', $child_id);
            $this->db->group_by('el.ss_aw_diagonastic_log_child_id');
            return $this->db->get()->result();
        } else {
    ...
    public function get_all_wright_answers_child($search_data = array(), $child_id = array(), $quiz_type, $question, $question_format = "", $question_id = "")
    {
        if ($quiz_type == 1) {
            ...
            if ($question_format == 'Multiple') {
                ...
                $this->db->where_in('mqa.ss_aw_child_id', $child_id);
                if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                    $this->db->where('DATE(mqa.ss_aw_created_at) >=', $search_data['start_date']);
                    $this->db->where('DATE(mqa.ss_aw_created_at) <=', $search_data['end_date']);
                }
                $this->db->group_by('mqa.ss_aw_child_id');
                return $this->db->get()->result();
            } elseif ($question_format == 'Single') {
