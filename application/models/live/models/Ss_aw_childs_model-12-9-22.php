...
public function deleterecord_byid($id)
	...
	public function get_daily_diagnostic_non_course_purchase_details($search_date){
		...
		$this->db->from($this->table);
		$this->db->join('ss_aw_parents','ss_aw_parents.ss_aw_parent_id = ss_aw_childs.ss_aw_parent_id');
		$this->db->where('(select count(*) from ss_aw_diagonastic_exam where ss_aw_diagonastic_exam.ss_aw_diagonastic_child_id = ss_aw_childs.ss_aw_child_id and DATE(ss_aw_diagonastic_exam.ss_aw_diagonastic_exam_date) <= "'.$search_date.'") > 0');
