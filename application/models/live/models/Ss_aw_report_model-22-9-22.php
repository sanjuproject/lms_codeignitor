...
class Ss_aw_report_model extends CI_Model
{
	...
	public function totallessonquesion($child_id = array(), $search_data = array(), $courseary = array()){
		...
		if ($quiz_type == 1) {
			...
			if (!empty($search_data['assign_level'])) {
				$this->db->where('ss_aw_lessons.ss_aw_course_id', $search_data['assign_level']);
			}
			else
			{
				if (!empty($courseary)) {
					$this->db->where_in('ss_aw_lessons.ss_aw_course_id', $courseary);
				}
			}
			
			/*if (!empty($search_data['update_date'])) {
				$this->db->where('DATE(ss_aw_lessons.ss_aw_lesson_created_date) <=', $search_data['update_date']);
			}*/
		...
		elseif ($quiz_type == 2){
			...
			if (!empty($search_data['assign_level'])) {
				if ($search_data['assign_level'] == 1) {
					$level = 'E';
				}
				elseif ($search_data['assign_level'] == 2) {
					$level = 'C';
				}
				else
				{
			...
			else
			{
				if (!empty($courseary)) {
					$course = array();
					foreach ($courseary as $key => $value) {
						if ($value == '1') {
							$course[] = 'E';
						}
						elseif ($value == '2') {
					...
					if (!empty($course)) {
						$this->db->where_in('ss_aw_level', $course);
					}	
				}
			}
			
			/*if (!empty($search_data['update_date'])) {
				$this->db->where('DATE(ss_aw_created_date) <=', $search_data['update_date']);
			}*/
	...
	public function fetchalllessonsquestions($child_id = array(), $search_data = array(), $limit, $start, $courseary){
		$quiz_type = $search_data['quiz_type'];
		if ($quiz_type == 1) {
		...
		elseif ($quiz_type == 2) {
			...
			if (!empty($search_data['assign_level'])) {
				...
				$this->db->where('ss_aw_assisment_diagnostic.ss_aw_level', $level);
			}
			else
			{
				if (!empty($courseary)) {
					...
					if (!empty($course)) {
						$this->db->where_in('ss_aw_assisment_diagnostic.ss_aw_level', $course);
					}
			...
			/*if (!empty($search_data['update_date'])) {
				$this->db->where('DATE(ss_aw_assisment_diagnostic.ss_aw_created_date) <=', $search_data['update_date']);
			}
			if (!empty($child_id)) {
				$child_id_str = implode(",", $child_id);
				$this->db->where('((select count(*) from ss_aw_assesment_multiple_question_answer where ss_aw_assesment_multiple_question_answer.ss_aw_child_id IN ('.$child_id_str.') and ss_aw_assesment_multiple_question_answer.ss_aw_question = ss_aw_assisment_diagnostic.ss_aw_question) != 0 or (select count(*) from ss_aw_assessment_exam_log where ss_aw_assessment_exam_log.ss_aw_log_child_id IN ('.$child_id_str.') and ss_aw_assessment_exam_log.ss_aw_log_question_id = ss_aw_assisment_diagnostic.ss_aw_id)) != 0');
	...
	public function totalnoofquestionasked($search_data = array(), $child_id = array(), $question, $quiz_type = "", $format = "", $question_id = ""){
		if ($quiz_type == 1) {
			$this->db->where('ss_aw_question', trim($question));
			if (!empty($child_id)) {
	...
	public function getPTDlessoncompletenum($date, $childs = array()){
		$this->db->where('DATE(ss_aw_last_lesson_modified_date)', $date);
		if (!empty($childs)) {
