...
class Ss_aw_coupons_model extends CI_Model
{
	...
	public function check_coupon_availability($coupon_code, $payment_type = "", $coupon_type = 0){
		$current_date = date('Y-m-d');
		$query = "'".$current_date."' BETWEEN ss_aw_start_date and ss_aw_end_date";
		$this->db->where('ss_coupon_code', trim($coupon_code));
		if (!empty($payment_type)) {
			$this->db->where('ss_aw_coupon_type', $payment_type);
		}
		$this->db->where('ss_aw_is_institution_coupon', $coupon_type);
		$this->db->where($query);
		return $this->db->get($this->table)->result();
	}
	...
	public function get_coupons_by_type($type){
		$this->db->where('ss_aw_status', 1);
		$this->db->where('ss_aw_default', 0);
		$this->db->where('ss_aw_coupon_type', $type);
		$this->db->where('ss_aw_is_institution_coupon', 1);
		return $this->db->get($this->table)->result();
	}
}