  ...
  class Ss_aw_last_readalong_model extends CI_Model
{
	...
	public function gettotalcompletenum($child_id = "", $course_start_date = ""){
		if (!empty($child_id) && !empty($course_start_date)) {
			...
			$this->db->where('ss_aw_last_readalong.ss_aw_create_date >', $course_start_date);
	...
	public function check_readalong_completion($readalong_id, $child_id, $start_date = "", $end_date = ""){
		$this->db->where('ss_aw_readalong_id', $readalong_id);
		$this->db->where('ss_aw_child_id', $child_id);
		if (!empty($start_date) && !empty($end_date)) {
			$this->db->where('DATE('.$this->table.'.ss_aw_create_date) >=', date('Y-m-d', strtotime($start_date)));
			$this->db->where('DATE('.$this->table.'.ss_aw_create_date) <=', date('Y-m-d', strtotime($end_date)));
		}
  ...
  public function get_complete_readalong_details($child_id, $start_date, $end_date, $finish_record_id = array()){
  	...
  	$this->db->from($this->table);
		...
		$this->db->where('DATE('.$this->table.'.ss_aw_create_date) >=', $start_date);
		$this->db->where('DATE('.$this->table.'.ss_aw_create_date) <=', $end_date);
	...
	public function get_total_complete_count($child_id, $course_id){
		$this->db->where('ss_aw_child_id', $child_id);
