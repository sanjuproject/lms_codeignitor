    ...
    public function totallessonquesion($child_id = array(), $search_data = array()) {
        ...
        if (!empty($child_id)) {
            ...
            if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                $this->db->where('(select count(*) from ss_aw_lesson_quiz_ans where ss_aw_lesson_quiz_ans.ss_aw_child_id IN (' . $child_id_str . ') '
                        . 'and DATE(ss_aw_lesson_quiz_ans.ss_aw_created_date) >= "' . $search_data['start_date'] . '" '
                        . 'and DATE(ss_aw_lesson_quiz_ans.ss_aw_created_date) <= "' . $search_data['end_date'] . '" '
                        . 'and ss_aw_lesson_quiz_ans.ss_aw_question_id = ss_aw_lessons.ss_aw_lession_id) != 0');
            } else {
                $this->db->where('(select count(*) from ss_aw_lesson_quiz_ans where ss_aw_lesson_quiz_ans.ss_aw_child_id IN (' . $child_id_str . ') '
                        . 'and ss_aw_lesson_quiz_ans.ss_aw_question_id = ss_aw_lessons.ss_aw_lession_id) != 0');
            }
            ...
            if (!empty($search_data['start_date']) && !empty($search_data['end_date'])) {
                $this->db->where('(select count(*) from ss_aw_lesson_quiz_ans where '
                        . 'DATE(ss_aw_lesson_quiz_ans.ss_aw_created_date) >= "' . $search_data['start_date'] . '" '
                        . 'and DATE(ss_aw_lesson_quiz_ans.ss_aw_created_date) <= "' . $search_data['end_date'] . '" '
                        . 'and ss_aw_lesson_quiz_ans.ss_aw_question_id = ss_aw_lessons.ss_aw_lession_id) != 0');
            } else {
                $this->db->where('(select count(*) from ss_aw_lesson_quiz_ans where '
                        . 'ss_aw_lesson_quiz_ans.ss_aw_question_id = ss_aw_lessons.ss_aw_lession_id) != 0');
            }
        }
        return $this->db->get()->num_rows();
    }

//        public function totallessonquesion_new($child_id = '', $search_data = array()){
//		$quiz_type = @$search_data['quiz_type'];
//		$this->db->select('ss_aw_lessons.ss_aw_lession_id,ss_aw_lessons.ss_aw_lesson_title,ss_aw_lessons.ss_aw_lesson_details');
//		$this->db->from('ss_aw_lessons');
//		$this->db->where('ss_aw_lessons.ss_aw_lesson_delete', 0);
...
//	}
    ...
    public function fetchalllessonsquestions($child_id = array(), $search_data = array(), $limit, $start) {
        ...
        $this->db->select('ss_aw_lessons.ss_aw_lesson_answers,ss_aw_lessons.ss_aw_lession_id,ss_aw_lessons.ss_aw_lesson_title,'
                . 'ss_aw_lessons.ss_aw_lesson_details,ss_aw_lessons.ss_aw_lesson_record_id,ss_aw_lessons.ss_aw_lesson_format,'
                . 'ss_aw_lessons.ss_aw_course_id,ss_aw_lessons_uploaded.ss_aw_course_id as course_level,ss_aw_lessons_uploaded.ss_aw_lesson_topic as topic_name');
    ...
    public function getPTDlessonAssesmentcompletenum($childs = array()) {
        $this->db->select('cl.ss_aw_child_id');
        $this->db->from('ss_aw_child_last_lesson cl');
        $this->db->join('ss_aw_assesment_uploaded au', 'au.ss_aw_lesson_id=cl.ss_aw_lesson_id AND au.ss_aw_assesment_status=1');
        $this->db->join('ss_aw_assessment_exam_completed ac', 'ac.ss_aw_assessment_id=au.ss_aw_assessment_id AND ac.ss_aw_child_id=cl.ss_aw_child_id');
        $this->db->where('DATE(ss_aw_last_lesson_modified_date) >=', report_date_from);
        if (!empty($childs)) {
            $this->db->where_in('cl.ss_aw_child_id', $childs);
        }
        $this->db->group_by(array('YEAR(ss_aw_last_lesson_modified_date)', 'MONTH(ss_aw_last_lesson_modified_date)', 'cl.ss_aw_child_id'));
...
SELECT cl.* FROM 
    ...
    `ss_aw_assesment_uploaded` `au` ON `au`.`ss_aw_lesson_id` = `cl`.`ss_aw_lesson_id`
        ...
        JOIN
    `ss_aw_assessment_exam_completed` `ac` ON `ac`.`ss_aw_assessment_id` = `au`.`ss_aw_assessment_id`
        AND `ac`.`ss_aw_child_id` = `cl`.`ss_aw_child_id`
WHERE
...
GROUP BY `cl`.`ss_aw_child_id`)tab WHERE 1 GROUP BY YEAR(ss_aw_last_lesson_modified_date) , MONTH(ss_aw_last_lesson_modified_date)")->row();
        return @$response->complete_num;
    }
...
//        $this->db->select('IF(count(*) = null, 0,count(*))as complete_num');
        $this->db->select('cl.*');
        $this->db->from('ss_aw_child_last_lesson cl');
        $this->db->join('ss_aw_childs', 'ss_aw_childs.ss_aw_child_id = cl.ss_aw_child_id');
        $this->db->where('cl.ss_aw_lesson_status', 2);
        $this->db->where('DATE(cl.ss_aw_last_lesson_modified_date) >=', report_date_from);
        ...
        if ($range_type == 1) {//active
            $this->db->where('DATEDIFF(now(),`cl`.`ss_aw_last_lesson_modified_date`)<', 15);
        } elseif ($range_type == 2) {//inactive
            $this->db->where('DATEDIFF(now(),`cl`.`ss_aw_last_lesson_modified_date`)>', 21);
        } elseif ($range_type == 3) {//deliquent
            $this->db->where('DATEDIFF(now(),`cl`.`ss_aw_last_lesson_modified_date`)>', 14);
            $this->db->where('DATEDIFF(now(),`cl`.`ss_aw_last_lesson_modified_date`)<', 22);
        }
        if (!empty($data['level'])) {
            $level = $data['level'];
            if ($level == 1) {
...
//        $this->db->group_by(array('MONTH(cl.ss_aw_last_lesson_modified_date)', 'YEAR(cl.ss_aw_last_lesson_modified_date)','cl.ss_aw_child_id'));
        $this->db->group_by(array('cl.ss_aw_child_id'));
...
un.ss_aw_section_title,
 (CASE WHEN(DATEDIFF(NOW(),IF((`cl`.`ss_aw_last_lesson_create_date`!=''),`cl`.`ss_aw_last_lesson_create_date`,de.ss_aw_diagonastic_exam_date)) > 7 AND DATEDIFF(NOW(),IF((`cl`.`ss_aw_last_lesson_create_date`!=''),`cl`.`ss_aw_last_lesson_create_date`,de.ss_aw_diagonastic_exam_date)) <= 14) THEN COUNT(cl.ss_aw_child_id)ELSE 0 END) first,
 (CASE WHEN(DATEDIFF(NOW(),IF((`cl`.`ss_aw_last_lesson_create_date`!=''),`cl`.`ss_aw_last_lesson_create_date`,de.ss_aw_diagonastic_exam_date)) > 14 AND DATEDIFF(NOW(),IF((`cl`.`ss_aw_last_lesson_create_date`!=''),`cl`.`ss_aw_last_lesson_create_date`,de.ss_aw_diagonastic_exam_date)) <= 29) THEN COUNT(cl.ss_aw_child_id)ELSE 0 END) second,
 (CASE WHEN(DATEDIFF(NOW(),IF((`cl`.`ss_aw_last_lesson_create_date`!=''),`cl`.`ss_aw_last_lesson_create_date`,de.ss_aw_diagonastic_exam_date)) > 29) THEN COUNT(cl.ss_aw_child_id)ELSE 0 END)thired,
 (CASE WHEN(DATEDIFF(NOW(),IF((`cl`.`ss_aw_last_lesson_create_date`!=''),`cl`.`ss_aw_last_lesson_create_date`,de.ss_aw_diagonastic_exam_date)) > 7) THEN COUNT(cl.ss_aw_child_id)ELSE 0 END)total 
  FROM
    ...
    SELECT 
        st.ss_aw_section_title,
            st.ss_aw_expertise_level,
            alu.ss_aw_lession_id,st.`ss_aw_section_reference_no`
    FROM
    ...
    WHERE ".
        $sql ." 
             UNION SELECT 
       lu.ss_aw_lesson_topic ss_aw_section_title,
            lu.ss_aw_course_id ss_aw_expertise_level,
           lu.ss_aw_lession_id,lu.`ss_aw_sl_no` ss_aw_section_reference_no
    FROM
       ss_aw_lessons_uploaded lu
    WHERE
        FIND_IN_SET('".$general_language."', lu.ss_aw_course_id))aun GROUP BY aun.ss_aw_lession_id)un
       JOIN
    ss_aw_child_last_lesson cl ON cl.ss_aw_child_id = de.ss_aw_diagonastic_child_id AND DATE(cl.`ss_aw_last_lesson_modified_date`)>='2022-08-01'
      AND cl.ss_aw_lesson_id = un.ss_aw_lession_id
       LEFT JOIN ss_aw_assessment_exam_completed aec ON aec.ss_aw_assessment_id = ((SELECT
                     aqs.`ss_aw_assessment_id`
             FROM
                 `ss_aw_assesment_questions_asked` aqs
            LEFT JOIN `ss_aw_assesment_uploaded` au ON au.`ss_aw_assessment_id` = aqs.`ss_aw_assessment_id`
             WHERE
                 au.ss_aw_lesson_id = un.ss_aw_lession_id 
                 AND aqs.ss_aw_child_id = de.ss_aw_diagonastic_child_id ORDER BY aqs.ss_aw_question_asked_id LIMIT 1)
                 UNION(SELECT
                     aqs.`ss_aw_assessment_id`
             FROM
                 `ss_aw_assesment_multiple_question_asked` aqs
            LEFT JOIN `ss_aw_assesment_uploaded` au ON au.`ss_aw_assessment_id` = aqs.`ss_aw_assessment_id`
             WHERE
                 au.ss_aw_lesson_id = un.ss_aw_lession_id 
                 AND aqs.ss_aw_child_id = de.ss_aw_diagonastic_child_id ORDER BY aqs.ss_aw_id LIMIT 1)) AND aec.ss_aw_child_id=de.ss_aw_diagonastic_child_id AND aec.ss_aw_create_date=''
      
       GROUP BY un.ss_aw_section_title ORDER BY un.ss_aw_section_reference_no");  
...
//		return $this->db->get($this->table)->result();
    ...
    public function getdiffuserlogintypenum_monthly($year, $month) {
        $this->db->select('(SELECT COUNT(*) FROM ss_aw_child_course '
                . 'WHERE YEAR(ss_aw_child_course.ss_aw_child_course_create_date) = "' . $year . '" '
                . 'and  MONTH(ss_aw_child_course.ss_aw_child_course_create_date) = "' . $month . '" '
                . 'and DATE(ss_aw_child_course.ss_aw_child_course_create_date) >= "' . report_date_from . '") as online');
        $this->db->limit(1);
        return $this->db->get()->result();
//		return $this->db->get($this->table)->result();
    }
