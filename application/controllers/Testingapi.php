...
class Testingapi extends CI_Controller
{
	...
	public function storescore($child_id, $level)
	{
		...
		if (!empty($scored_lesson_list)) {
			...
			foreach ($scored_lesson_list as $key => $value) {
				...
				$data['ss_aw_lesson_quiz_correct_percentage'] = (float)$percentage;
				...
				$data['ss_aw_assessment_in_level_actual_score'] = (float)$actual_score;
				$data['ss_aw_assessment_in_level_potential_score'] = (float)$potential_score;
				$data['ss_aw_assessment_in_level_score'] = (float)$in_leve_precentage;
				...
				$data['ss_aw_assessment_next_level_actual_score'] = (float)$nxt_actual_score;
				$data['ss_aw_assessment_next_level_potential_score'] = (float)$nxt_potential_score;
				$data['ss_aw_assessment_next_level_score'] = (float)$nxt_level_precentage;
				...
				$data['ss_aw_review_correct_percentage'] = (float)get_percentage($review_asked, $review_correct);
				...
				$combine_percentage = (float)get_percentage($assessment_actual_score, $assessment_potential_score);
	...
	public function lesson_assessment_completion_view()
	{
		...
		$data['assessment_list'] = $topical_assessments;
	...
	public function send_update_notification()
	{
		...
		$file_link = base_url() . 'assets/templates/app-update-notification.html';
		$content = file_get_contents($file_link);

		$receiver_emails = array();
		$parent_emails = array();
		$child_emails = array();
		$parents = $this->db->get('ss_aw_parents')->result();
		if (!empty($parents)) {
			foreach ($parents as $key => $value) {
				$parent_emails[] = $value->ss_aw_parent_email;
			}
		}
		$childs = $this->db->get('ss_aw_childs')->result();
		if (!empty($childs)) {
			foreach ($childs as $key => $value) {
				$child_emails[] = $value->ss_aw_child_email;
			}
		}
		$receiver_emails = array_merge($parent_emails, $child_emails);
		array_unique($receiver_emails);
		array_values($receiver_emails);
		if (!empty($receiver_emails)) {
			for ($i = 0; $i < count($receiver_emails); $i++) {
	...
	public function create_pdf_export_new($child_id)
	{
		...
		if ($child_array->result_id->num_rows>0) {
	...
	public function change_readalong_schedule_table_data()
	{
		$result = $this->db->get('ss_aw_schedule_readalong')->result();
		if (!empty($result)) {
			foreach ($result as $key => $value) {
				$child_id = $value->ss_aw_child_id;
				$childary = $this->ss_aw_child_course_model->get_details($child_id);
				$course_id = $childary[count($childary) - 1]['ss_aw_course_id'];
				$this->db->set('ss_aw_course_id', $course_id);
				$this->db->where('ss_aw_id', $value->ss_aw_id);
				$this->db->update('ss_aw_schedule_readalong');
			}
		}
	}

	public function change_readalong_complete_table_data()
	{
		$result = $this->db->get('ss_aw_last_readalong')->result();
		if (!empty($result)) {
			foreach ($result as $key => $value) {
				$child_id = $value->ss_aw_child_id;
				$childary = $this->ss_aw_child_course_model->get_details($child_id);
				...
				$this->db->set('ss_aw_course_id', $course_id);
				$this->db->where('ss_aw_id', $value->ss_aw_id);
				$this->db->update('ss_aw_last_readalong');
			}
		}
	}
	...
	public function check_template()
	{
		$data = array();
		$this->load->library('pdf');
		$html = $this->load->view('pdf/invoice_html', $data, true);
		$filename = time() . ".pdf";
		$this->pdf->viewPDF("", $html, $filename, false);
	}

	public function finish_readalong($child_id, $readalong_id)
	{
		$this->load->model('ss_aw_readalong_quiz_model');
		...
		$readalong_quiz = $this->ss_aw_readalong_quiz_model->get_record_by_upload_id($readalong_id);

		if (!empty($readalong_quiz)) {
			foreach ($readalong_quiz as $key => $value) {
				$question_id = $value['ss_aw_readalong_id'];
				$data = array(
					'ss_aw_child_id' => $child_id,
					'ss_aw_quiz_id' => $question_id,
					'ss_aw_readalong_id' => $readalong_id,
					'ss_aw_quiz_ans_post' => "",
					'ss_aw_quiz_right_wrong' => 2
				);
				$this->ss_aw_readalong_quiz_ans_model->data_insert($data);
			}
			//mark complete in last readalong table
			$data = array();
			$data['ss_aw_child_id'] = $child_id;
			$data['ss_aw_readalong_id'] = $readalong_id;
			$data['ss_aw_status'] = 1;
			$data['ss_aw_course_id'] = $course_id;
			$this->ss_aw_last_readalong_model->data_insert($data);

			//mark taken in schedule readalong table
			$data = array();
			$data['ss_aw_child_id'] = $child_id;
			$data['ss_aw_readalong_id'] = $readalong_id;
			...
			$this->db->where('ss_aw_child_id', $child_id);
			$this->db->update('ss_aw_schedule_readalong', $data);

			$responseary = array(
				'status' => 200,
				'msg' => 'Mark as completed successfully.'
			);
			die(json_encode($responseary));
		}
	}

	public function remove_not_completed_readalong_details()
	{
		$this->db->where('ss_aw_readalong_status', 2);
		$this->db->where('ss_aw_child_id !=', 371);
		$this->db->where('DATE(ss_aw_create_date) < ', '2023-09-04');
		$not_completed_readalongs = $this->db->get('ss_aw_schedule_readalong')->result();
		if (!empty($not_completed_readalongs)) {
			foreach ($not_completed_readalongs as $key => $value) {
				$child_id = $value->ss_aw_child_id;
				$readalong_id = $value->ss_aw_readalong_id;

				//remove schedule table record
				$this->db->where('ss_aw_child_id', $child_id);
				$this->db->where('ss_aw_readalong_id', $readalong_id);
				$this->db->delete('ss_aw_schedule_readalong');

				//remove readalong page store record
				$this->db->where('ss_aw_child_id', $child_id);
				$this->db->where('ss_aw_readalong_id', $readalong_id);
				$this->db->delete('ss_aw_store_readalong_page');

				//remove readalong quiz record
				$this->db->where('ss_aw_child_id', $child_id);
				$this->db->where('ss_aw_readalong_id', $readalong_id);
				$this->db->delete('ss_aw_readalong_quiz_ans');
				...
				$responseary = array(
					'status' => 200,
					'msg' => 'Pending readalongs removed successfully.'
				);
				die(json_encode($responseary));
			}
		}
	...
	{
		$list = $this->ss_aw_lessons_uploaded_model->get_course_lesson_list(1);
		echo $this->db->last_query();
		echo "<pre>";
		print_r($list);
		die();
		$this->db->query("SELECT * FROM ss_aw_assesment_uploaded WHERE ss_aw_assesment_topic_id IN (SELECT GROUP_CONCAT(ss_aw_section_id) FROM ss_aw_sections_topics WHERE FIND_IN_SET('C', ss_aw_expertise_level) AND ss_aw_topic_deleted = 1 AND ss_aw_section_status = 1) AND ss_aw_assesment_status = 1 AND ss_aw_assesment_delete = 0")->result();

		echo $this->db->last_query();
	}

	public function send_maintenance_notification()
	{
		$this->db->where('ss_aw_child_delete', 0);
		$this->db->where('ss_aw_child_status', 1);
		$result = $this->db->get('ss_aw_childs')->result();

		$email_template = getemailandpushnotification(68, 1, 1);
		$body = $email_template['body'];
		$this->load->model('ss_aw_email_que_model');
		if (!empty($email_template)) {
			foreach ($result as $key => $value) {
    ...
    // public function get_institutional_masters_users(){
    // 	$query = "select * from ss_aw_childs where ss_aw_institution_user_upload_id != 0 and (select ss_aw_course_id from ss_aw_child_course where ss_aw_child_course.ss_aw_child_id = ss_aw_childs.ss_aw_child_id order by ss_aw_child_course_id desc limit 1) = 5";
    // 	$result = $this->db->query($query)->result();
    // 	if (!empty($result)) {
    // 		foreach ($result as $key => $value) {
    // 			$this->db->where('ss_aw_child_id', $value->ss_aw_child_id);
    // 			$this->db->set('ss_aw_is_self', 1);
    // 			$this->db->update('ss_aw_childs');
    // 		}
    // 	}
    // }

    public function test_image(){
    	$a = file_get_contents("http://54.81.49.130/public/generators/1706601618043-bezkoder-avatar.png");
    	print_r($a);
    }
	...
	// public function get_mlp_pdf_report($child_id)
	// {
	// 	$this->load->model("ss_aw_child_assessment_pdf");
	// }	
	public function mlp_performance_report()
	{
		...
		$data['mlp_user'] = $this->ss_aw_child_assessment_pdf->get_mlp_user_data();
		$this->load->view('admin/headerunauthorised', $headerdata);
		$this->load->view('mlp/mlp_performance_report', $data);
	}
	...
	{
		if ($child_id == "") {
			$child_id = $this->input->post("mlp_user");
		}


		$child_array = $this->ss_aw_child_assessment_pdf->get_all_mlp_course_completed_child($child_id);
		...
		if (!empty($child_array)) {
			foreach ($child_array->result() as $child) {
				...
				if ($child->course_level == "E" || $child->course_level == "M") { //for grammer
					$start = 1;
					$end = 11;
					$grammer_array = $this->ss_aw_child_assessment_pdf->get_child_grammer_assessment_mlp($child->ss_aw_child_id, $start, $end);
				...
				$vocabulary_array = $this->ss_aw_child_assessment_pdf->get_child_grammer_assessment_mlp($child->ss_aw_child_id, $start, $end);
				...
				$data['total_voc_assesment_actual'] = 0;
				$data['course_details_grammer_first'] = $grammer_first;
				$data['course_details_grammer_second'] = $grammer_second;
				$data['course_details_grammer_third'] = $grammer_third;
				$data['course_details_vocabulary_first'] = $voca_first;
				...
				$this->load->library('pdf');
				$html = $this->load->view('pdf/finalscore_new_mlp', $data, true);
			

				if (!file_exists("./scorepdf/mlp_report/")) {
					mkdir("./scorepdf/mlp_report/", 0777);
				}
				$filename = time() . rand() . "_" . $child->ss_aw_child_id . ".pdf";
				$save_file_path = "./scorepdf/mlp_report/" . $filename;
				...
				$this->pdf->createPDF($save_file_path, $html, $filename, false);
				// $this->pdf->Output($save_file_path, 'I');
				echo json_encode($save_file_path);
				exit;
			}
		}
	}
