 ...
 * 
 */
class Report_05_05_2023 extends CI_Controller {
    ...
    public function lesson_question_difficulty2() {
        ...
        if ($this->session->userdata('lesson_search_data')) {
            ...
            if (!empty($searchdata['student_level'])) {
                $students = $this->ss_aw_child_course_model->getstuentsbycorsetype($searchdata['student_level']); //child id comma

                if (!empty($students->child_id)) {
                    $child_id = explode(",", $students->child_id); //all student data according to lesson.
                }
            }
...
//            exit;
        ...
        $student_level = $this->session->userdata('student_level');
    ...
    public function inactive_stage() {
        ...
        $winners_topics = $this->ss_aw_sections_topics_model->get_all_records(15, 0);
        ...
        $champions_topics = $this->ss_aw_sections_topics_model->get_all_records(10, 15);
        ...
        $masters_topics = $this->ss_aw_sections_topics_model->get_all_records();
    ...
    public function advance_receipts() {
        ...
        if ($this->input->post()) {
            ...
            $this->session->unset_userdata('advance_receipts_start_date');
            $this->session->unset_userdata('advance_receipts_end_date');
        ...
        if (!empty($result)) {
            foreach ($result as $key => $value) {
                ...
                $month = date('m', strtotime($value->ss_aw_created_at));
                $revenue = $this->ss_aw_reporting_revenue_model->getdatabyparentmonth($parent_id, $month);
                $reporting_revenue[$parent_id][$month] = $revenue[0]->ss_aw_invoice_amount;
            }
        }

        $data['reporting_collection'] = $result;
        $data['reporting_revenue'] = $reporting_revenue;
    ...
    public function advance_receipts_export() {
        ...
        if (!empty($this->session->userdata('advance_receipts_start_date')) && !empty($this->session->userdata('advance_receipts_end_date'))) {
            $postdata = $this->input->post();
            $start_date = date('m/Y', strtotime($this->session->userdata('advance_receipts_start_date')));
            $end_date = date('m/Y', strtotime($this->session->userdata('advance_receipts_end_date')));
            $result = $this->ss_aw_reporting_collection_model->getalldata($start_date, $end_date);
        } else {
            $start_date = date('m');
            $end_date = date('Y');
        ...
        $pagedata['reporting_collection'] = $result;
        $pagedata['reporting_revenue'] = $reporting_revenue;
        ...
        foreach ($result as $value) {
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, '');
            $objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $program_type);
            $objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $invoice_amount);
            $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $value->ss_aw_payment_type == 0 ? "Lumpsum" : "EMI");
            $objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $value->ss_aw_discount_amount);
            $gst = round(($invoice_amount * 18) / 100, 2);
            $objPHPExcel->getActiveSheet()->SetCellValue('L' . $rowCount, $gst);
            $objPHPExcel->getActiveSheet()->SetCellValue('M' . $rowCount, $invoice_amount + $gst);
            $rowCount++;
        }
