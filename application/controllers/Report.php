...
class Report extends CI_Controller
{
	...
	public function lesson_question_difficulty()
	{
		...
		if (!empty($student_level)) {
			...
			if ($student_level == 1 || $student_level == 2) {
				...
				$topic_listary = $this->ss_aw_sections_topics_model->get_all_records(5,25);
			}
			$topicAry = array();
		...
		$pagedata['courses_drop'] = $this->common_model->getAllcourses();
	...
	public function retention_score_report()
	{
		...
		$data['courses_drop'] = $this->common_model->getAllcourses();
	...
	public function assessment_wrong_answer_childs_list()
	{
		...
		$assessment_format = $urlStringAry[2];
		$pageno = $urlStringAry[3] ? $urlStringAry[3] : 0;
		$age_level = $urlStringAry[4] ? $urlStringAry[4] : '';

		$pagedata['page_num'] = $pageno;
		if ($student_level) {
			$child_id_ary = array();
			$students = $this->ss_aw_child_course_model->getstuentsbycorsetype($student_level, $age_level);
			$child_id_ary = array();
			if (!empty($students->child_id)) {
				$child_id_ary = explode(",", $students->child_id);
			}
