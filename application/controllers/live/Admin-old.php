...
class Admin extends CI_Controller
{
	...
	public function childrendetails()
	{
		...
		$result = $this->ss_aw_childs_model->get_all_child_details($parent_id);
		$lessoncount = array();
					...
					if ($inputpost['emi_payment']) {
						...
						if (!empty($inputpost['promoted'])) {
							...
							$reporting_revenue_data = array(
								...
								'ss_aw_bill_no' => $invoice_no,
								'ss_aw_course_id' => $previous_course_id,
								'ss_aw_invoice_amount' => round($first_revenue_amount),
								'ss_aw_discount_amount' => round($discount_amount),
								'ss_aw_revenue_date' => $today_date,
							...
							while ($course_duration != 0) {
								if ($count == 0) {
									...
									$reporting_revenue_data = array(
										...
										'ss_aw_bill_no' => $invoice_no,
										'ss_aw_course_id' => $course_id,
										'ss_aw_invoice_amount' => round($second_revenue_amount),
										'ss_aw_discount_amount' => 0,
										'ss_aw_revenue_date' => $today_date,
										'ss_aw_revenue_count_day' => $remaing_days,
										'ss_aw_payment_type' => $postdata['emi_payment']
        ...
        if (isset($_FILES["file"]['name'])) {
			...
			if ($this->input->post('programme_type') == 1) {
				if ($header_field_count != 6) {
					$this->session->set_flashdata('error', 'Please choose the correct format for winners programme.');
					redirect($_SERVER['HTTP_REFERER']);
				}
			}else{
				if ($header_field_count != 4) {
					$this->session->set_flashdata('error', 'Please choose the correct format for masters programme.');
					redirect($_SERVER['HTTP_REFERER']);
				}
			}
	...
	public function institution_individual_performance(){
		...
		if ($programme_type == 1) {
			...
			//get all childs
			$total_record = $this->ss_aw_childs_model->total_winner_users($institution_users_id);
			...
			$childs = $this->ss_aw_childs_model->get_users_by_parents_ary($institution_users_id, $config['per_page'], $page);	
		}
		elseif ($programme_type == 3) {
			...
			$topic_listary = $this->ss_aw_sections_topics_model->get_all_records(0, 0, $search_data);
			//get all childs
			$total_record = $this->ss_aw_childs_model->total_masters_users($institution_users_id);
			$redirect_to = base_url() . 'admin/institution_individual_performance/'.$institution_id;
			$uri_segment = 4;
			$record_per_page = 10;
			$config = pagination_config($redirect_to, $total_record, $uri_segment, $record_per_page);
			...
			$childs = $this->ss_aw_childs_model->get_master_users_by_parents_ary($institution_users_id, $config['per_page'], $page);
		}
		$topicAry = array();
					...
					foreach ($lesson_listary as $lesson_topic) {
						...
						if (!empty($lesson_complete_date) && !empty($prev_assessment_complete_date)) {
							...
							if (!empty($check_finish)) {
								...
								$readalongscore[$child_id][$lesson_topic['ss_aw_lession_id']] = $readalong_score;
						...
						$assessment_completetion_details = $this->ss_aw_assessment_exam_completed_model->get_assessment_completion_details($assessment_id, $child_id);
						if (!empty($assessment_completetion_details)) {
							$prev_assessment_complete_date = $assessment_completetion_details[0]->ss_aw_create_date;
						}
						//end		
					}
				}
		...
		$data['readalongscore'] = $readalongscore;
		$data['readalong_id_ary'] = $readalong_id_ary;
		$data['institution_details'] = $this->ss_aw_institutions_model->get_record_by_id($institution_id);
		$this->load->view('admin/header', $headerdata);
					...
					foreach ($lesson_listary as $i => $lesson_topic) {
						...
						if (!empty($childs)) {
							foreach ($childs as $key => $value) {
								...
								//diagnostic section
								$diagnostic_exam_code_details = $this->ss_aw_diagonastic_exam_model->get_exam_details_by_child($child_id);
								if (!empty($child_course_details) && !empty($diagnostic_exam_code_details)) {
									...
									if (!empty($child_activation_date)) {
										...
										if ($accessable) {
											...
											if(!empty($lesson_complete_date)){
												$readalong_start_date = $lesson_complete_date;
												$restriction_detail = $this->ss_aw_readalong_restriction_model->get_restricted_time($programme_type);
												$restriction_time = $restriction_detail[0]->restricted_time;
												$readalong_end_date = date('Y-m-d H:i:s', strtotime($readalong_start_date. "+ ".$restriction_time." minutes"));
												//get scheduled readalong details
				...
				foreach ($childs as $key => $value) {
					...
					if ($payment_type == 1) {
						...
						if (!empty($register_email_template)) {
							...
							$body = str_ireplace("[@@password@@]", $password, $body);
							$body = str_ireplace("[@@email@@]", $value->ss_aw_child_email, $body);
							$body = str_ireplace("[@@username@@]", $value->ss_aw_child_nick_name, $body);
							$send_data = array(
								'ss_aw_email' => $value->ss_aw_child_email,
								'ss_aw_subject' => 'Welcome to ALSOWISE. Thank you for registering with us.',
