			...
			if($this->check_child_existance($child_id,$child_token)) // Check provider Token valid or not against child_id and token
			{
				...
				*/
				...
				if($child_level == 'E')
				{
					...
					{
						// for first 5 questions
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('C');
						$responseary = $this->updatelevel($child_id,'C',$topic_count,5,1,$diagonastic_exam_code);	//$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
					else if($correct_ans == 8 && $total_ans < 15) // When Correct Answes  8
					{
						// for first 5 questions
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('E');
						$responseary = $this->updatelevel($child_id,'E',$topic_count,5,2,$diagonastic_exam_code); //$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
					else if($correct_ans < 8 && $total_ans < 15) // When Correct Answes 6
					{
						// for first 5 questions
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('E');
						$responseary = $this->updatelevel($child_id,'E',$topic_count,5,4,$diagonastic_exam_code); //$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
					else if($correct_ans >= 13 && $total_ans > 14) // When Correct Answes 13 or more
					{
						// for second 5 questions
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('C');
						$responseary = $this->updatelevel($child_id,'C',$topic_count,5,3,$diagonastic_exam_code);	 //$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
				...
				else if($child_level == 'C')
				{
					if($correct_ans > 8 && $correct_ans < 11 && $total_ans < 15) // When Correct Answes more than 8
					{
						//done
						//for first 5 questions.
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('A');
						$responseary = $this->updatelevel($child_id,'A',$topic_count,5,1,$diagonastic_exam_code);	//$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
					else if($correct_ans == 8 && $total_ans < 15) // When Correct Answes  8
					{
						//done
						//for first 5 questions.
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('C');
						$responseary = $this->updatelevel($child_id,'C',$topic_count,5,2,$diagonastic_exam_code); //$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
					else if($correct_ans < 8 && $total_ans < 15) // When Correct Answes 6
					{
						//done
						//for first 5 questions.
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('C');
						$responseary = $this->updatelevel($child_id,'C',$topic_count,5,2,$diagonastic_exam_code, $correct_ans, $total_ans); //$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
					else if($correct_ans >= 13 && $total_ans > 14) // When Correct Answes 13 or more
					{
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('A');
						$responseary = $this->updatelevel($child_id,'A',$topic_count,5,3,$diagonastic_exam_code);	 //$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code
					}
					else if($correct_ans < 13 && $total_ans > 14) // When Correct Answes more than 10 bet less than 13
					{
						//done
						//for second 5 questions.
						$topic_count = $this->ss_aw_sections_topics_model->get_topiccount_bylevel('C');
						$responseary = $this->updatelevel($child_id,'C',$topic_count,5,4,$diagonastic_exam_code); //$child_id,$level,$category_count,$question_no,$student_level,diagonastic_exam_code	
					}
				...
				if($response)
				{
					...
					if (!empty($lesson_listary)) {
						...
						foreach ($lesson_listary as $key => $value) {
							if (strtolower($value['ss_aw_lesson_format']) == 'single') {
								$serial_ary[$value['ss_aw_lession_id']] = $lesson_count;
								$lesson_count++;	
							}
							else{
								$lesson_name_ary = explode(" ", $value['ss_aw_lesson_topic']);
								$serial_ary[$value['ss_aw_lession_id']] = $lesson_name_ary[count($lesson_name_ary) - 1];
							}
						}
					}
					$serial_no = 0;
				...
				$responseary['msg'] = 'Readalong completed successfully';
					...
					if (!empty($parent_detail)) {
						...
						if (!empty($email_template)) {
							...
							$body = str_ireplace("[@@username@@]", $parent_detail[0]->ss_aw_parent_full_name, $body);
							$body = str_ireplace("[@@child_name@@]", $child_name, $body);
							$body = str_ireplace("[@@gender@@]", $gender_name, $body);
							$send_data = array(
								'ss_aw_email' => $parent_detail[0]->ss_aw_parent_email,
								'ss_aw_subject' => $email_template['title'],
								'ss_aw_template' => $body,
				...
				*/
				...
				if(!empty($result))
				{
					...
					foreach($result as $key=>$value)
					{
						...
						$complete_count = $this->ss_aw_assessment_exam_completed_model->totalcompletebytopic($value['ss_aw_assesment_topic'], $child_id);
     ...
     if($inputpost)
     {
			...
			if($this->check_child_existance($child_id,$child_token)) // Check provider Token valid or not against child_id and token
			{
				...
				if(empty($response))
				{
					...
					/*$searchary = array();
					$searchary['ss_aw_course_id'] = $level;
					$assessment_list = $this->ss_aw_assesment_uploaded_model->fetch_by_params($searchary);*/
					//$assessment_list = $this->ss_aw_assesment_uploaded_model->get_assessment_bylevel($level);
					/*$serial_ary = array();
					...
					//code for checking promotion eligibility
					$course_code = $level;
					$count_complete_assessment = $this->ss_aw_assessment_score_model->getcountbychildcourse($child_id, $course_code);
					$minimum_assessment_complete_num = "";
					$maximum_assessment_complete_num = "";
					if ($level == 'E') {
						$minimum_assessment_complete_num = 4;
						$maximum_assessment_complete_num = 6;
					}
					elseif ($level == 'C') {
						$minimum_assessment_complete_num = 13;
						$maximum_assessment_complete_num = 15;
					}
					if (!empty($minimum_assessment_complete_num) && (count($count_complete_assessment) >= $minimum_assessment_complete_num && count($count_complete_assessment) <= $maximum_assessment_complete_num)) {
						$check_promotional_invitation = $this->ss_aw_promotion_model->checkpromotionlapsation($child_id, $level);
						if ($check_promotional_invitation == 0) {
						$TOT_QUIZ_CNT_C = 0;
						$PER_SUM_SCO_E = 0;
						$PER_SUM_SCO_C = 0;
						...
						if ($TOT_QUIZ_CNT_C > TOT_QUIZ_CNT_C && $PER_SUM_SCO_E >= PER_SUM_SCO_E && $PER_SUM_SCO_C >= PER_SUM_SCO_C) {
								$data = array(
									'ss_aw_child_id' => $child_id,
								...
								if ($response) {
									...
									if ($course_code == 'E') {
										if ($childary[count($childary) - 1]['ss_aw_course_payemnt_type'] == 1) {
											$email_template = getemailandpushnotification(34, 1, 2);
											$app_template = getemailandpushnotification(34, 2, 2);
											$action_id = 27;	
										}
										else{
										...
										if ($childary[count($childary) - 1]['ss_aw_course_payemnt_type'] == 1) {
											$email_template = getemailandpushnotification(46, 1, 2);
											$app_template = getemailandpushnotification(46, 2, 2);
											$action_id = 29;	
										}
										else{
											$email_template = getemailandpushnotification(45, 1, 2);
											$app_template = getemailandpushnotification(45, 2, 2);
											$action_id = 28;
										}
									}
									...
									if (!empty($parent_detail)) {
										...
										if (!empty($email_template)) {
											...
											$body = str_ireplace("[@@gender@@]", $g, $body);
											$send_data = array(
												'ss_aw_email' => $parent_detail[0]->ss_aw_parent_email,
												'ss_aw_subject' => $email_template['title'],
												'ss_aw_template' => $body,
			...
			if($this->check_child_existance($child_id,$child_token))
				{
					...
					if($purchased_courseary[0]['ss_aw_selected_course_id'] == 1)
						$current_course = "E";
					else if($purchased_courseary[0]['ss_aw_selected_course_id'] == 2)
						$current_course = "C";
   	...
   	$inputpost = $this->input->post(); // Accept All post data post from APP		
		...
		if($inputpost)
		{
			...
			if($this->check_child_existance($child_id,$child_token)) // Check provider Token valid or not against child_id and token
			{
				...
				if ($total_lesson > 0) {
					if ($level == 1) {
						//Below code is applicable for the original version
						/*if (($total_lesson % 2) == 1) {
							$fetch_lesson_num = $total_lesson;
						}
						else
						...
						//Below code is applicable onlym for FUT version
						$fetch_lesson_num = $total_lesson;	
					}
					else
					{
