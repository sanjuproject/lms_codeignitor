...
class Report extends CI_Controller
{
	...
	public function lesson_question_difficulty(){
		...
		if (!empty($result)) {
			...
			foreach($result as $key => $value){
				...
				$asked_question_details = $this->ss_aw_report_model->totalnoofquestionasked($searchdata, $child_id, $value->ss_aw_lession_id, 1);
				$log_ids = array();
				if (!empty($asked_question_details)) {
					foreach ($asked_question_details as $log_details) {
						$log_ids[] = $log_details->ss_aw_id;
					}
				}
				$question_asked[$value->ss_aw_lession_id] = count($log_ids);
				$correct_answer[$value->ss_aw_lession_id] = $this->ss_aw_report_model->totalnoofcorrectanswer($searchdata, $child_id, $value->ss_aw_lession_id, 1, "", $log_ids);
				$incorrect_answer_count[$value->ss_aw_lession_id] = $this->ss_aw_report_model->totalnoofincorrectanswer($searchdata, $child_id, $value->ss_aw_lession_id, 1, "", $log_ids);
	...
	public function assessment_question_difficulty(){
		...
		if ($this->session->userdata('assessment_search_data')) {
			...
			$upload_ids = $this->ss_aw_report_model->get_assessment_upload_ids($topic);
			$sq_ary = array();
			if (!empty($upload_ids)) {
				foreach ($upload_ids as $value) {
					$topic_questions = $this->ss_aw_report_model->get_assessment_question_by_upload_id($value->ss_aw_uploaded_record_id);
					if (!empty($topic_questions)) {
						$sq_no = 1;
						foreach ($topic_questions as $questions) {
							$sq_ary[$questions->ss_aw_id] = $sq_no;
							$sq_no++;
						}
					}
				}
			...
			$total_record = $this->ss_aw_report_model->totalassessmentquestion($child_id, $searchdata);
			...
			$result = $this->ss_aw_report_model->fetchallassessmentquestions($child_id, $searchdata, $config['per_page'], $page);
		}
		...
		if (!empty($result)) {
			foreach($result as $key => $value){
				...
				$asked_question_details = $this->ss_aw_report_model->totalnoofquestionasked($searchdata, $child_id, $question, 2, $value->ss_aw_format, $value->ss_aw_id);
				$log_ids = array();
				if (!empty($asked_question_details)) {
					foreach ($asked_question_details as $log_details) {
						$log_ids[] = $log_details->log_id;
					}
				}
				$question_asked[$value->ss_aw_id] = count($log_ids);
				$correct_answer[$value->ss_aw_id] = $this->ss_aw_report_model->totalnoofcorrectanswer($searchdata, $child_id, $question, 2, $value->ss_aw_format, $value->ss_aw_id, $log_ids);
				$incorrect_answer_count[$value->ss_aw_id] = $this->ss_aw_report_model->totalnoofincorrectanswer($searchdata, $child_id, $question, 2, $value->ss_aw_format, $value->ss_aw_id, $log_ids);
				...
				if (!empty($sq_ary[$value->ss_aw_id])) {
					$question_sequence_no[$value->ss_aw_id] = $sq_ary[$value->ss_aw_id];
				}
				else{
					$question_sequence_no[$value->ss_aw_id] = 0;
				}
	...
	public function diagnostic_question_difficulty(){
		...
		if ($this->session->userdata('diagnostic_search_data')) {
			...
			$total_record = $this->ss_aw_report_model->totaldiagnosticquestion($child_id, $searchdata);
			...
			$result = $this->ss_aw_report_model->fetchalldiagnosticquestions($child_id, $searchdata, $config['per_page'], $page);
		}
	...
	public function track_readalong_selection(){
		...
		if ($this->input->post()) {
			...
			$track_readalong_data['age'] = $age;
			if (!empty($assign_level)) {
				$search_data = array();
				if ($assign_level == 1) {
					$level = "E";
				}
				elseif ($assign_level == 3 || $assign_level == 5) {
					$level = "A";
				}
				...
				if (!empty($readalongs)) {
					foreach ($readalongs as $key => $value) {
						...
						$selected_num_ptd[$value['ss_aw_id']] = $this->ss_aw_report_model->getreadalongselectionnum($readalong_id,'',$age);
						$selected_num_ytd[$value['ss_aw_id']] = $this->ss_aw_report_model->getreadalongselectionnum($readalong_id, 1,$age);
						$selected_num_mtd[$value['ss_aw_id']] = $this->ss_aw_report_model->getreadalongselectionnum($readalong_id, 2,$age);

						$completed_num_ptd[$value['ss_aw_id']] = $this->ss_aw_report_model->getreadalongcompletednum($readalong_id,'',$age);
						$completed_num_ytd[$value['ss_aw_id']] = $this->ss_aw_report_model->getreadalongcompletednum($readalong_id, 1,$age);
						$completed_num_mtd[$value['ss_aw_id']] = $this->ss_aw_report_model->getreadalongcompletednum($readalong_id, 2,$age);
					}
				}	
			}
		}
	...
	public function export_readalong_selection_report(){
		...
		$this->load->library('pdf');
		$html = $this->load->view('pdf/report/track_readalong_selection_report', $data, true);
		$filename = time().".pdf";
		$save_file_path = "./scorepdf/".$filename;
		$this->pdf->viewPDF($save_file_path, $html, $filename, false);
	}
	...
	public function enroll_current_status(){
		...
		if ($this->input->post()) {
			$postdata = $this->input->post();
			$searchdata['level'] = $postdata['assign_level'];
			$data['assign_level'] = $postdata['assign_level'];
		}

		$result = get_active_inactive_delinquent_students($searchdata);
		// Create data of program to date.
		$program_date = date('Y-m-d');
		$ptd_total_num = 0;
		$ptd_total_num_percentage = 0;
		$ptd_active_num = 0;
		$ptd_active_num_percentage = 0;
		$ptd_inactive_num = 0;
		$ptd_inactive_num_percentage = 0;
		$ptd_delinquent_num = 0;
		$ptd_delinquent_num_percentage = 0;

		$PTD_total_complete_num = $this->ss_aw_report_model->getPTDlessoncompletenum();
		if (!empty($result['all_childs_ary'])) {
			$ptd_total_num = $this->ss_aw_report_model->getPTDlessoncompletenum( $result['all_childs_ary']);
			if ($PTD_total_complete_num > 0) {
				$ptd_total_num_percentage = ( ( $ptd_total_num / count($result['all_childs_ary']) ) * 100 );	
			}	
		}

		if (!empty($result['active_student_ary'])) {
			$ptd_active_num = count($result['active_student_ary']);	
			{
				$ptd_active_num_percentage = ( ( count($result['active_student_ary']) / count($result['all_childs_ary']) ) * 100 );	
			}
		}

		if (!empty($result['inactive_student_ary'])) {
			$ptd_inactive_num = count($result['inactive_student_ary']);
			{
				$ptd_inactive_num_percentage = ( ( count($result['inactive_student_ary']) / count($result['all_childs_ary']) ) * 100 );	
			}	
		}

		if (!empty($result['delinquent_student_ary'])) {
			$ptd_delinquent_num = count($result['delinquent_student_ary']);
			{
				$ptd_delinquent_num_percentage = ( ( count($result['delinquent_student_ary']) / count($result['all_childs_ary'])) * 100 );	
			}	
		}
		
		$data['ptd_total_num'] = $ptd_total_num;
		$data['ptd_total_num_percentage'] = round($ptd_total_num_percentage, 2);
		$data['ptd_active_num'] = $ptd_active_num;
		$data['ptd_active_num_percentage'] = round($ptd_active_num_percentage, 2);
		$data['ptd_inactive_num'] = $ptd_inactive_num;
		$data['ptd_inactive_num_percentage'] = round($ptd_inactive_num_percentage, 2);
		$data['ptd_delinquent_num'] = $ptd_delinquent_num;
		$data['ptd_delinquent_num_percentage'] = round($ptd_delinquent_num_percentage, 2);

		// End of program to date.
		$count = 0;
		$monthly_data = array();
		$current_month = date('m');
		$current_year = date('Y');
		...
		$total_monthly = array();
		$active_monthly = array();
		$delinquent_monthly = array();
		$inactive_monthly = array();
		...
		for ($i=0; $i <= 25; $i++) {
			...
			$monthly_total_complete_num = $this->ss_aw_report_model->getmonthlylessoncompletenum($current_month, $current_year);
			// All childs details
			$total_monthly['complete_num'][$i] = 0;	
			$total_monthly['complete_num_percent'][$i] = 0;
			if (!empty($result['all_childs_ary'])) {
				$total_monthly['complete_num'][$i] = $this->ss_aw_report_model->getmonthlylessoncompletenum($previous_month, $previous_year, $result['all_childs_ary']);
				if ($monthly_total_complete_num > 0) {
					$total_monthly['complete_num_percent'][$i] = ( ( $total_monthly['complete_num'][$i] / $monthly_total_complete_num ) * 100 ); 	
				}	
			}

			// Active childs details
			$active_monthly['complete_num'][$i] = 0;
			$active_monthly['complete_num_percent'][$i] = 0;
			if (!empty($result['active_student_ary'])) {
				$active_monthly['complete_num'][$i] = $this->ss_aw_report_model->getmonthlylessoncompletenum($previous_month, $previous_year, $result['active_student_ary']);
				if ($monthly_total_complete_num > 0) {
					$active_monthly['complete_num_percent'][$i] = ( ( $active_monthly['complete_num'][$i] / $monthly_total_complete_num ) * 100 );
					$active_monthly['complete_num_percent'][$i] = round($active_monthly['complete_num_percent'][$i], 2);
				}
				 	
			}

			// Inactive childs details
			$inactive_monthly['complete_num'][$i] = 0;
			$inactive_monthly['complete_num_percent'][$i] = 0;
			if (!empty($result['inactive_student_ary'])) {
				$inactive_monthly['complete_num'][$i] = $this->ss_aw_report_model->getmonthlylessoncompletenum($previous_month, $previous_year, $result['inactive_student_ary']);
				
				if ($monthly_total_complete_num > 0) {
					$delinquent_monthly['complete_num_percent'][$i] = ( ( $inactive_monthly['complete_num'][$i] / $monthly_total_complete_num ) * 100 );	
					$delinquent_monthly['complete_num_percent'][$i] = round($delinquent_monthly['complete_num_percent'][$i], 2);
				}	
			}

			// Deliquent childs details
			$delinquent_monthly['complete_num'][$i] = 0;
			$delinquent_monthly['complete_num_percent'][$i] = 0;
			if (!empty($result['delinquent_student_ary'])) {
				$delinquent_monthly['complete_num'][$i] = $this->ss_aw_report_model->getmonthlylessoncompletenum($previous_month, $previous_year, $result['delinquent_student_ary']);
				
				if ($monthly_total_complete_num > 0) {
					$delinquent_monthly['complete_num_percent'][$i] = ( ( $delinquent_monthly['complete_num'][$i] / $monthly_total_complete_num ) * 100 );	
					$delinquent_monthly['complete_num_percent'][$i] = round($delinquent_monthly['complete_num_percent'][$i], 2);
				}	
			}
		...
		$data['active_monthly'] = $active_monthly;
		$data['monthly_data'] = $monthly_data;
		$data['delinquent_monthly'] = $delinquent_monthly;
		$data['inactive_monthly'] = $inactive_monthly;
	...
	public function days_deliquent(){
		...
		if ($this->input->post()) {
			$postdata = $this->input->post();
			$searchary['age'] = $postdata['age'];
			$searchary['level'] = $postdata['level'];
		}
		$result = get_delinquent_students($searchary);
		$lesson_list = $this->ss_aw_report_model->getlessonwithassociateassessment();
		$complete_data_num = array();
		if (!empty($lesson_list)) {
			foreach ($lesson_list as $key => $value) {
				$complete_data_num['first'][$value['lesson_id']] = $this->ss_aw_report_model->getcompletelessonassessmentcount($result['delinquent_student_ary']['first'], $value['lesson_id'], $value['assessment_id']);
				$complete_data_num['second'][$value['lesson_id']] = $this->ss_aw_report_model->getcompletelessonassessmentcount($result['delinquent_student_ary']['second'], $value['lesson_id'], $value['assessment_id']);
				$complete_data_num['third'][$value['lesson_id']] = $this->ss_aw_report_model->getcompletelessonassessmentcount($result['delinquent_student_ary']['third'], $value['lesson_id'], $value['assessment_id']);
				$complete_data_num['last'][$value['lesson_id']] = $this->ss_aw_report_model->getcompletelessonassessmentcount($result['delinquent_student_ary']['last'], $value['lesson_id'], $value['assessment_id']);
			}
		}

		$data['complete_data'] = $complete_data_num;
		$data['lesson_list'] = $lesson_list;
