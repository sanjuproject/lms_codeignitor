    ...
    public function getExcelfileName($master_data) {
        ...
        if (!empty($master_data)) {
            foreach ($master_data->result() as $mas) {
                if (!empty($mas->ss_aw_last_lesson_modified_date) && !empty($mas->ss_aw_create_date) && empty($mas->ss_aw_mlp_access_given_id) && $mas->upcoming_lession_id != $mas->ucoming_lesson_id) {
                    $row_count++;
                    $row++;
                    $col = 0;
...
//        $objWriter->save('php://output');
    ...
    public function active_cheat_code($child_id_encode) {
        ...
        $topical_lessons = $this->ss_aw_lessons_uploaded_model->get_lessonlist_by_topics_check_last_lesson($topicAry, $child_id);
