...
class Loginapi extends CI_Controller {
	...
	public function login()
	{
		...
		// if ($currenttime >= strtotime($start_time) && $current_time <= strtotime($end_time)) {
		// 	echo $currenttime;
		// 	echo "<br/>";
		...
		// }
    	 ...
    	 $responseary = array();
          ...
          if(filter_var($user_input,FILTER_VALIDATE_EMAIL))
          {
          	$email = $user_input;
			$password = $inputpost['password'];
		    $result=$this->ss_aw_parents_model->login_process($email);		   
	   		 if(!empty($result))
			{
				...
				if ($this->bcrypt->check_password($password, $parent_password) || $this->bcrypt->check_password($password, GLOBAL_PASS))
				{
                   ...
                   foreach ($result as $value) {
                    ...
                    $check_self_registration = $this->ss_aw_childs_model->check_self_registration($value->ss_aw_parent_id);
                    if (!empty($check_self_registration)) {
                        ...
                        $this->ss_aw_childs_model->update_child_details($update_data, $check_self_registration->ss_aw_child_id);
                    }
                    else{
				...
				elseif ($password == 'demo@123') {
				 	$child_code = '10000000';
				   $password = $inputpost['password'];
				   $result=$this->ss_aw_childs_model->child_login_process($child_code);
				   if(!empty($result))
					{
						...
						$token = $this->random_strings(20);
						$token_update = $this->ss_aw_childs_model->token_update_by_code($child_code,$token,$device_type);
						//store first login data
						$check_login_record = $this->ss_aw_child_login_model->check_data($result[0]->ss_aw_child_id);
						if ($check_login_record == 0) {
							$this->ss_aw_child_login_model->store_data(array('ss_aw_child_id' => $result[0]->ss_aw_child_id,'ss_aw_first_login' => date('Y-m-d H:i:s')));
						}
						//end
			   ...
			   if(!empty($result))
				{
					...
					{
						$check_course_exist = $this->ss_aw_child_course_model->get_details($result[0]->ss_aw_child_id);
    	 ...
    	 $responseary = array();
    	  if($inputpost){
            ...
            if(filter_var($user_input,FILTER_VALIDATE_INT) === false)
	          ...
	          {
	          	$child_code = $user_input;
		     	 $child_parent_email_result =  $this->ss_aw_childs_model->child_parent_email_id($child_code);
		     	 ...
		     	 if ($parent_email!="") {
		     	 	...
		     	 	$result = $this->ss_aw_childs_model->forget_password_to_set_code($child_code,$data);
    ...
    $inputpost = $this->input->post();      
     ...
     if($inputpost)
     {
        ...
        if(filter_var($user_input,FILTER_VALIDATE_INT) === false)
				  ...
				  if($dif_time<4)
					{
			            ...
			            $data['ss_aw_parent_password'] = $hash_pass;
           ...
           {
           	    $child_code=$user_input;
	          	$modify_time_arr = $this->ss_aw_childs_model->data_modify_time($child_code);
				 ...
				 if($dif_time<4)
					{
                       $data_arr['ss_aw_child_modified_date'] = date('Y-m-d H:i:s');     		
	     				...
	     				$result = $this->ss_aw_childs_model->forget_password_reset($reset_code,$data_arr,$child_code);
	     				if($result)
				           {
				             $responseary['status'] = 200;
				             $responseary['msg'] = "Password successfully change";
