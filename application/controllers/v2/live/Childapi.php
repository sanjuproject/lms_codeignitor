	...
	public function schedule_readalongs()
	{
		...
		if ($inputpost) {
			...
			if ($parent_id == "" ? $this->check_child_existance($child_id, $child_token) : $this->check_parent_existance($parent_id, $child_token)) // Check provider Token valid or not against child_id and token
			{
				...
				$inputpost['start_date'] = date('Y-m-d 00:00:00', strtotime($inputpost['start_date']));
		...
		if ($inputpost) {
			...
			if ($parent_id == "" ? $this->check_child_existance($child_id, $child_token) : $this->check_parent_existance($parent_id, $child_token)) // Check provider Token valid or not against child_id and token
			{
				...
				if (!empty($last_schedule_readalong)) {
					$chek_duplicate_question = $this->ss_aw_readalong_quiz_ans_model->ceck_duplicate_question($child_id, $inputpost['readalong_id'], $inputpost['question_id'], $last_schedule_readalong[0]->ss_aw_start_date, $last_schedule_readalong[0]->ss_aw_end_date);
			...
			if ($parent_id == "" ? $this->check_child_existance($child_id, $child_token) : $this->check_parent_existance($parent_id, $child_token)) // Check provider Token valid or not against child_id and token
			{
				...
				if ($total_lesson > 0) {
					...
					if (!empty($get_lesson_completion_detail)) {
						...
						$end_date = date('Y-m-d H:i:s', strtotime($start_date . "+ " . $restriction_time . " minutes"));
						$current_time = time();
						if (($current_time >= strtotime($start_date)) && ($current_time <= strtotime($end_date))) {
							if ($level == 1 || $level == 2) {
