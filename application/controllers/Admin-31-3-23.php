...
class Admin extends CI_Controller
{
	...
	public function manage_institutions(){
		...
		if ($this->input->post()) {
			...
			$config["total_rows"] = $this->ss_aw_institutions_model->number_of_records();
			$config["per_page"] = 10;
			$config["uri_segment"] = 3;
			$config['full_tag_open'] = '<ul class="pagination pagination-rounded justify-content-end">';
			$config['full_tag_close'] = '</ul>';
			...
			$str_links = $this->pagination->create_links();
			$data["links"] = explode('&nbsp;', $str_links);
			$data['institutions'] = $this->ss_aw_institutions_model->get_all_records($config['per_page'], $page);
			$institution_users = array();
			if (!empty($data['institutions'])) {
				foreach ($data['institutions'] as $key => $value) {
					$institution_users[$value->ss_aw_id] = $this->ss_aw_parents_model->get_institutions_admin($value->ss_aw_id);
				}
			}
			$data['institution_users'] = $institution_users;
			$data['page'] = $page;
			$data['countries'] = $this->ss_aw_countries_model->get_all_records();
		...
		if ($this->input->post()) {
			...
			elseif ($check_duplicate_mobile > 0) {
				...
				$institution_update_response = $this->ss_aw_institutions_model->update_record($institution_data, $update_record_id);
				{
					...
					$response = $this->ss_aw_parents_model->update_record_by_institution($user_data, $update_record_id);
