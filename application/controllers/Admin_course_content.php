	...
	public function readalongs()
	{
		...
		if($this->input->post())
		{
			...
			*/
			...
			if (!empty($postdata['demo_readalong_availability'])) {
				$pageno = $postdata['demo_pageno'];
				$demo_readalong_id = $postdata['demo_readalong_id'];
				$is_demo = 0;
				if (isset($postdata['is_demo'])) {
					if (count($postdata['is_demo']) > 1) {
						$is_demo = 3;
				...
				$update_array = array();
				$update_array['ss_aw_is_demo'] = $is_demo;
				$this->ss_aw_readalongs_upload_model->update_record($demo_readalong_id,$update_array);
				if ($is_demo == 1) {
					//schedule for winners
					$child_code = "10000000";
					$child_details = $this->ss_aw_childs_model->check_child_existance($child_code);
					$child_id = $child_details[0]->ss_aw_child_id;
					$schedule_readalong = array();
					$schedule_readalong['ss_aw_child_id'] = $child_id;
					$schedule_readalong['ss_aw_readalong_id'] = $demo_readalong_id;
					$schedule_readalong['ss_aw_schedule_readalong'] = date('Y-m-d H:i:s');
					$schedule_readalong['ss_aw_start_date'] = date('Y-m-d H:i:s');
					$schedule_readalong['ss_aw_end_date'] = date('Y-m-d H:i:s', strtotime($schedule_readalong['ss_aw_start_date']." + 7 day"));
					$this->ss_aw_schedule_readalong_model->data_insert($schedule_readalong);
				}
				elseif ($is_demo == 2) {
				...
				elseif ($is_demo == 3) {
					...
					$child_details = $this->ss_aw_childs_model->check_child_existance($child_code);
					$child_id = $child_details[0]->ss_aw_child_id;
					$remove_schedule = array();
					$remove_schedule['ss_aw_child_id'] = $child_id;
					$remove_schedule['ss_aw_readalong_id'] = $demo_readalong_id;
					$this->ss_aw_schedule_readalong_model->delete_data($remove_schedule);
				...
				$this->session->set_flashdata('success','Readalong demo status succesfully change.');
				redirect('admin_course_content/readalongs/'.$pageno);
			}
